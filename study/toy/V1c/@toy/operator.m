...
%
%               op = operator(S,'position');   % position operator
%               op = operator(S,'X');          % position operator
%               op = operator(S,'x');          % position operator
%
%               op = operator(S,'natural');    % natural operator
%               op = operator(S,'N');          % natural operator
%               op = operator(S,'n');          % natural operator
%
      ...
      switch arg
         ...
         case {'X','x','position'}
            ...
            for (i=1:length(labs))
               ...
               d = sscanf(lab,'%f');
               s = sprintf('%g',d);
               if strcmp(s,lab)      % can we convert string to a double?
                  x(i) = d;
               else
                  break;
               end
            end
            if (length(x) ~= n)   % complete set of position eigen values?
               x = 1:n;           % otherwise fall back to the default
            end
            M = sparse(diag(x));
         case {'#','natural'}
            M = sparse(diag(1:n));
