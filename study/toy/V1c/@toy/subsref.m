      ...
      case '()'
         ...
         if iscell(arg)
            ...
            switch type(obj)
               ...
               case '#CSPACE'
                  if (length(arg) ~= 1)
                     error('index list of eigenvalues must contain exactly 1 number!');
                  end
                  out = VectorByEigenValue(obj,arg);
                  if isempty(out)
               ...
               otherwise
                  switch type(obj)
                     ...
                     case '#CSPACE'
                        ...
                        elseif iscell(arg)
                           if length(arg) ~= 1
                              error('index list must contain exactly 1 eigen value!');
                           end
                           out = VectorByEigenValue(obj,arg);
                           if isempty(out)
                              error(['unsupported eigen value for indexing: ',arg]);
                           end
                        else
                           error('index to a cspace must be either double or list!');
                        end
                     case '#SPLIT'
...
%==========================================================================
% Get Vector by Eigen Value
%==========================================================================

function V = VectorByEigenValue(obj,sym)
%
% VECTOR-BY-EIGEN-VALUE    Get vector by eigen value. Return empty matrix
%                          if not successful
%
   try
      V = vector(obj,sym);
