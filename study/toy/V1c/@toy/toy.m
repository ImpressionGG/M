...
%       T = type(toy,'#WAVE')        % generic '#WAVE'
%       T = type(toy,'#SPLIT')       % generic '#SPLIT'
%       T = type(toy,'#UNIVERSE')    % generic '#UNIVERSE'
%       T = type(toy,'#HISTORY')     % generic '#HISTORY'
%
...
%
%    3) Special quantum toys are created by providing 1 or 2 input args. These
%       calls are delegate to method setup (see also: SETUP)
%
%       H = toy('spin');             % create 2D spin space
%       H = toy('alpha');            % create alpha decay toy model
%
%       H = toy('coin');             % create quantum coin space
%       H = toy('die');              % create quantum die space
%       H = toy('game');             % create quantum game space
...
%       version       version of TOY toolbox
...
% Start with special toy creation (1 or 2 input args)
   ...
   elseif (nargin == 2)
      H = space(toy,varargin{1});       % setup a quantum toy
   ...
   if (~done && n <= 3)
      ...
      if ischar(typ)
         switch typ
            ...
            case '#CSPACE'
               if (n < 3)
                  dat.cspace.index = {};      % list of eigen values
                  dat.cspace.list = {};       % list of eigen vector matrices
                  dat.cspace.olist = {};      % list of observables
                  dat.cspace.observable = []; % empty observable
                  dat.cspace.basis = [];
               end
               done = 1;
...
% Finally we have to create the toy object. This is done by creating
% a parent object (CORE) and deriving a TOY object as a derived class 
      
   parent = core(typ,par,dat);
   o = class(struct([]),lower(mfilename),parent);     
   o = type(o,typ,mfilename);                        % provide tag
   ...
   if (nargout==0 && either(property(o,'generic?'),0))
      shell(o);
   else
      out = o;
   end
