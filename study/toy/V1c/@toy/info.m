...
%
% INFO     Get info of a toy object
%
%               S = space(toy,{'u','d'},[1 1;-1 1]/sqrt(2));
%               P = projector(S,'u');
%
%               txt = info(S);
%               txt = info(P);
...
%
   spc = iif(either(opt(obj,'space'),0),' ','');
   ...
   switch typ
      ...
      case {'#CSPACE','#WAVE'}
         ...
         txt = '';
         if length(index) > 0
            txt = sprintf('%g',length(index{1}));
            for (i=2:length(index))
               txt = [txt,sprintf('x%g',length(index{i}))];
            end
         end
         txt1 = txt;
