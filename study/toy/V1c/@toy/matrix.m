...
%
% MATRIX     Get or set type matrix
%
%            1) for a Hilbert space object ('#SPACE') return the matrix
%            of basis vectors.
%
%               S = space(type,{'u','d'},[1 1;-1 1]/sqrt(2));
%               M = matrix(S);
%
...
%
   ...
   if (length(varargin) == 0)
      switch typ
         ...
         otherwise
            error(['matrix method does not support type ''',typ,'''!']);
      end
      profiler('matrix',0);
      return
   end
...
% set type matrix
   ...
   if (length(varargin) == 1)
      ...
      switch typ
         ...
         otherwise
            error(['cannot apply matrix setting to this type type: ',typ]);
      end
      profiler('matrix',0);
      return
   end
...
% set matrix elements
   ...
   if (length(varargin) > 1)
      switch typ
         ...
         otherwise
            error(['cannot apply setting of matrix elements to this type type: ',typ]);
      end
      profiler('matrix',0);
      return
   end
