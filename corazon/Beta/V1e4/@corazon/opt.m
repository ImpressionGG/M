   ...
   switch nargin
      ...
      case 2
         ...
         if iscell(tag)
            ...
            if (ischar(tag) && ~isempty(find(tag==',')))
               varargout = prop(o,{'work.opt'},varargin{1});
               return
            end
            value = opt(o,tag);
            if isempty(value)
         ...
         elseif ischar(tag)
            ...
            if ~isfield(wrk,'opt')
               ...
               if isempty(idx)
                  if ~isempty(find(tag==','))
                     varargout = prop(o,{'work.opt'},tag);
                     return
                  elseif isfield(opts,tag)
                     value = opts.(tag);
                  else
                     value = [];
                  end
      ...
      otherwise
         ...
         elseif (nargout == 1)         % multiple set
            tag = varargin{1};
            if iscell(tag) && ~isempty(tag)
               tags = tag{1};
               if (ischar(tags) && ~isempty(find(tags==',')))
                  varargout{1} = prop(o,{'work.opt'},tag,varargin(2:end));
                  return
               end
            elseif (ischar(tag) && ~isempty(find(tag==',')))
               varargout{1} = prop(o,{'work.opt'},tag,varargin(2:end));
               return
