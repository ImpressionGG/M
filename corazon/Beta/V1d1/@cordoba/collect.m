...
%       table0 = {{@read,'cordoba','PackageDrv','.pkg'},...
%                 {@read,'cordoba','UniLogDrv', '.log'}};
...
%       collect(o,{})                  % reset collection config
%
%       table1 = {{@read,'cordoba','PackageDrv','.pkg'},...
%                 {@read,'cordoba','UniLogDrv', '.log'}};
%       collect(o,{'pln','smp'},table1); 
%
%       table2 = {{@read,'cordoba','PackageDrv','.pkg'},...
%                 {@read,'cordoba','UniLogDrv', '.log'}};
%                 {@basis,'cordoba','VibTxtDrv','.txt'}},...
%       collect(o,{'vib'},table2); 
%
%       table3 = {{@read,'cordoba','PackageDrv','.pkg'},...
%                 {@read,'cordoba','UniLogDrv', '.log'}};
%                 {@basis,'cordoba','BmcTxtDrv','.txt'}},...
%                 {@basis,'cordoba','BmcPbiDrv','.pbi'}},...
%       collect(o,{'bmc'},table3); 
%
%    Copyright(c): Bluenetics 2020 
%
%    See also: CORDOBA, IMPORT
%
...
function list = Import(o,directory,table)
   ...
   for (k=1:length(table))
      ...
      suffix = entry{4};               % supported file suffix
         ...
         % scan all files of directory with matching extension
      ...
      for (i=1:length(files))
         ...
         path = o.upath([directory,'/',file,ext]);
         ...
         if isequal(oo.type,'pkg')
            ...
            system  = get(oo,{'system',system});
            kind    = get(oo,{'kind',kind});
            sizes   = get(oo,{'sizes',sizes});
            method  = get(oo,{'method',method});
            pitch   = get(oo,{'pitch',pitch});
