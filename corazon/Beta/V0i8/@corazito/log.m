function varargout = log(o,varargin)           % Create Log Object or Log To   
% 
% LOG   Data logging
%
...
%
   while (nargin > 1 && isa(varargin{1},'double'))
      ...
      nmax = size(varargin{1},2);
      for (i=2:length(tags))
         if (size(varargin{i},2) ~= nmax)
            error('all data added to log object must have same number of columns!');
         end
      end
...
%   
   ...
   while (1)
      ...
      elseif (c == ',')
         ...
         c = Get('impossible error!');
         continue;               % separator
      elseif IsLetter(c) || (c == '_') || IsDigit(c) && ~isempty(sym)
         sym(end+1) = c;
