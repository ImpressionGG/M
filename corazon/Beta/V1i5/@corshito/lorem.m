function [title,text] = lorem(mode,from,to)
%
% LOREM   Lorem Ipsum text generator
%
%            [title,text] = lorem
%            [title,text] = lorem(mode,from,to)
%
...
%
   ...
   switch mode
      otherwise
         ...
         idx(jdx) = [];
         if ~isempty(idx)
            title = title(1:max(idx));
         end
         
         for (i=1:length(title))
            if ~isempty(find(title(i)=='.,;'))
               title(i) = '$';
            end
         end
   ...
   title = lower(corazon.trim(title));
   title = cordoba.capital(title);
   text = text(:)';
end

function [item,line,list] = Ipsum      % Raw Text                      
   list = {{'Lorem ipsum dolor sit amet,'
           ...
           {'Aenean commodo ligula eget dolor.'
            ...
            {'Nunc nec neque. Phasellus leo dolor,'  
             'tempus non, auctor et, hendrerit quis,'   
             'nisi. Curabitur ligula sapien, tincidunt non,'
             'euismod vitae, posuere imperdiet,'
            ...
            {'Vestibulum fringilla pede sit amet augue.'
             'In turpis. Pellentesque posuere.'
            }
            {'Praesent turpis. Aenean posuere,'  
             'tortor sed cursus feugiat,'
            ...
            {'Nullam sagittis. Suspendisse pulvinar,'
             'augue ac venenatis condimentum,' 
             'sem libero volutpat nibh,'
            ...
            {'Proin sapien ipsum, porta a, auctor quis,'
             'euismod ut, mi.'
            ...
            {'Ut non enim eleifend felis pretium feugiat.'
             'Vivamus quis mi. Phasellus a est.'   
            }};
   idx = Random(1,length(list));
   item = list{idx};
   
   jdx = Random(1,length(item));
   line = item{jdx};
end
function n = Random(from,to)           % Random Number in Given Range  
   delta = to-from;
   while (1)
      n = from + floor(rand*(delta+1));
      if n >= from && n <= to
         return
      end
   end
