...
%
%              pbi(cordoba)            % register PBI plugin
%
...
function o = Import(o)                 % Import from File              
   oo = mitem(o,'PBI');
   ooo = mitem(oo,'PBI Data File (.dat)',{@ImportCb,@ReadPbiDat,'.dat',@cordoba});
   ooo = mitem(oo,'PBI Data File (.txt)',{@ImportCb,@ReadPbiTxt,'.txt',@cordoba});
   ooo = mitem(oo,'PBI Data File (.pbi)',{@ImportCb,@ReadPbiPbi,'.pbi',@cordoba});
   ...
   function o = ImpfortCb(o)           % Import Log Data Callback      
      ...
      %o = cordoba(o);                 % finally want create a CORDOBA obj
...
function o = Collect(o)                % Configure Collection          
   table = {{@read,'cordoba','ReadPkgPkg','.pkg'},...
            ...
            {@pbi,'cordoba','ReadPbiTxt', '.txt'},...
            {@pbi,'cordoba','ReadPbiPbi', '.pbi'}};
   collect(o,{'pbi'},table); 

   table = {{@read,'cordoba','ReadPkgPkg','.pkg'},...
            {@read,'cordoba','ReadGenDat', '.dat'},...
            {@pbi,'cordoba','ReadMbcTxt', '.txt'},...
            {@pbi,'cordoba','ReadMbcPbi', '.pbi'}};
   collect(o,{'mbc'},table); 
end
...
%
% READ   Read a CORDOBA object (type BMC/VIB) from file.
%
%           oo = Read(o,'PbiTxtDrv',path)
%           oo = Read(o,'PbiPbiDrv',path)
%           oo = Read(o,'NewPbiPbiDrv',path)
%
%        See also: CORDOBA, IMPORT, WRITE
%
...
%
   ...
   o = opt(o,'category.units',{'�','m�'});

   o = category(o,1,[-2 2],[0 0],'�');        % category 1 for x,y
