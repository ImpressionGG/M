...
%             default('gain',1.0);
%             men = uimenu(gcf,'label','Gain',UD,'gain');
%                   Choice(o,men,{{'Auto',[]},[0.5 1.0 1.5]},CHCR);
%
%             default('spin','UP');
%             men = uimenu(gcf,'label','Spin',UD,'spin');
...
%
   ...
   o.work.mitem = itm;                 % store parent item handle in work
   if (isa(list,'double'))
      array = list(:);
   ...
   elseif (iscell(list))
      for (i=1:length(list))
         ...
         elseif (iscell(entry))
            ...
            %uimenu(itm,LB,label,CB,callback,UD,value);
            mitem(o,label,callback,value);
         elseif isa(entry,'double')
            entry = entry(:);
            for (j=1:length(entry))
               value = entry(j);  label = sprintf('%g',value);  
               mitem(o,label,callback,value);
            end
         else
            error('bad list item!');
