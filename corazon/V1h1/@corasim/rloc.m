...
%
%                          +----------+      
%                          |    p(s)  |          p(s) = prod((s-zeros(i))
%              ---->o----->| K*------ |------*----> 
%                   ^ -    |    q(s)  |      |   q(s) = prod((s-poles(i))
%                   |      +----------+      |
%                   |          L(s)          |
%                   +------------------------+
%
%                    L(s)            K*p(s)/q(s)            K*p(s)
%          T(s) = ----------  =  -----------------  =  -----------------
%                  1 + L(s)       1 + K*p(s)/q(s)       q(s) + K*p(s)
%                                                       :...........:        
%                                                         := c(s)
%
%       Closed loop poles locations: => root locations of c(s)
%
%       Options
%
...
%
   ...
   if (nargin == 3)
      ...
      o = RlocTrf(o,num,den);          % plot root locus
   else
      o = Auto(o);                     % auto setting of plot range
      o = RlocSs(o);                   % plot root locus
   end
...
function o = RlocTrf(o,num,den)        % Plot Root Locus of Trf        
   ...
   %       
      Kmini = opt(o,{'kmin',-inf});
      Kmaxi = opt(o,{'kmax',+inf});
      ...
      if (length(K) >= 2)
         ...
         % is this a k-range we should really handle?
         
      if (Kmax >= Kmaxi && K >= Kmaxi)
         return
      end
      if (Kmax <= Kmini && K <= Kmini)
         return
      end
      ...
      for (k=1:5000)
         [r,K] = TrfRoots(o,num,den,z,p,r,K,Kmax,delta);
         x(:,end+1) = real(r);
         y(:,end+1) = imag(r);
...
function o = RlocSs(o)                 % Plot Root Locus of SS         
   col = {'b','bc','r','m','rw','mw','rww','mww'};
   K = [-inf 0 1 1.5 2 5 10 20];
...
%  [z,p,k] = zpk(o,num,den);
   ...
   if (opt(o,{'pentagon',1}))
      for (i=3:length(col))
         r = Roots(A,B,C,D,K(i));      % closed loop roots for K = 1
         plot(o,real(r),imag(r),[col{i},'p']);
      end
   end
   ...
   %       
      ...
      for (k=1:500)
         if (ss)
            [r,K] = SsRoots(o,A,B,C,D,z,p,r,K,Kmax,delta);
         else
            [r,K] = TrfRoots(o,num,den,z,p,r,K,Kmax,delta);
         end
...
function [r,K] = TrfRoots(o,num,den,z,p,r,K,Kmax,delta)                
...
function [r,K] = SsRoots(o,A,B,C,D,z,p,r,K,Kmax,delta)                 
   delta = ones(length(r),1)*delta;
...
function r = Roots(A,B,C,D,K);         % Closed Loop Roots             
   I = eye(size(D));
   AK = A - B*inv(I+K*D)*K*C;
   r = eig(AK);
end
