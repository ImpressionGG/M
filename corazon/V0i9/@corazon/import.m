...
function list = Append(o,oo,list)      % append Data Object            
   ...
   msg = sprintf(['%s seems to be already loaded, which would ',...
                  'lead to duplicate objects! ',...
                  'Skip or terminate (Cancel)?'],title);
          
   button = questdlg(msg,'Duplicate Object!','Skip','Cancel','Skip');
               
   if isequal(button,'Duplicate')      % so far not supported!
      list{end+1} = oo;                % add object to list
   elseif isequal(button,'Replace')
      list{end+1} = {oo};              % embedding indicates replacement
   elseif isequal(button,'Cancel')
      list = [];                       % return non cell to terminate
   end
end