...
%
%    Syntax
%       oo = context(oo,path)               % extract total avail. context
%
...
%
%    See also: CORAZON, EXTRACT, ID, READ, IMPORT, COLLECT
%
   if (nargin == 2)
      oo = All(o,varargin{1});
      return
...
function oo = All(o,path)
   oo = o;
   [dir,file,ext] = fileparts(path);
   
      % set package id, if available
      
   [ID,number,machine,digits] = id(o,dir);
   if ~isempty(machine) && ~isempty(digits)
      package = ['@',machine,'.',digits];
      oo = set(oo,'package',package);
   end
   
      % extract parameter settings from file name
      
   oo = extract(oo,file);
   
      % extract project identifier from path
      
   project = context(o,'Project',path);
   if ~isempty(project)
...
%
   co = corleon;                       % for access of CORLEON methods
   list = {};
%
