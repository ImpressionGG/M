...
%
%         See also: CORDOBA, SAMPLE, PBI, DANA, KEFALON
%
   if (nargin == 0)
...
function o = Import(o)                 % add Import/Basis Menu         
   oo = mitem(o,'Basis');
   ooo = mitem(oo,'Vibration Test (.txt)',{@ImportCb,@ReadVibTxt,'.txt',@cordoba});
   ooo = mitem(oo,'Vibration Test (.dat)',{@ImportCb,@ReadVibDat,'.dat',@cordoba});
   ooo = mitem(oo,'Vibration Test (.vib)',{@ImportCb,@ReadVibVib,'.vib',@cordoba});
   ooo = mitem(o,'-');
   ooo = mitem(oo,'BMC Test (.txt)',{@ImportCb,@ReadBmcTxt,'.txt',@cordoba});
   ooo = mitem(oo,'BMC Test (.dat)',{@ImportCb,@ReadBmcDat,'.dat',@cordoba});
   return
...
function o = Export(o)                 % add Export/Basis Menu         
   ...
   ooo = mitem(oo,'Vibration Test (.txt)',{@ExportCb,'WriteVibTxt','.txt',@cordoba});
   set(mitem(ooo,inf),'enable',onoff(o,'vib'));
   ooo = mitem(oo,'Vibration Test (.dat)',{@ExportCb,'WriteVibDat','.dat',@cordoba});
   set(mitem(ooo,inf),'enable',onoff(o,'vib'));
   ooo = mitem(oo,'-');
   ooo = mitem(oo,'BMC Test (.txt)',{@ExportCb,'WriteBmcTxt','.txt',@cordoba});
   set(mitem(ooo,inf),'enable',onoff(o,'bmc'));
   ooo = mitem(oo,'BMC Test (.dat)',{@ExportCb,'WriteBmcDat','.dat',@cordoba});
   set(mitem(ooo,inf),'enable',onoff(o,'bmc'));
   return
...
function o = Collect(o)                % Configure Collection          
   table = {{@read,'cordoba','ReadPkgPkg','.pkg'},...
            {@basis,'cordoba','ReadVibDat','.dat'},...
            {@basis,'cordoba','ReadVibTxt','.txt'},...
            {@basis,'cordoba','ReadVibVib','.vib'}};
   collect(o,{'vib'},table); 

   table = {{@read,'cordoba','ReadPkgPkg','.pkg'},...
            {@basis,'cordoba','ReadBmcDat','.dat'},...
            {@basis,'cordoba','ReadBmcTxt','.txt'},...
            {@basis,'cordoba','ReadBmcPbi','.pbi'}};
   collect(o,{'bmc'},table); 
end
...
%oo = opt(oo,opt(o));
   ...
   function o = Config(o)              % Setup Proper Plot Config      
      ...
      o = category(o,1,[-2.0 2.0],[0 0],'�');
      o = category(o,2,[-0.5 0.5],[0 0],'�');
