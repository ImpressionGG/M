...
%
%    Check if object is a trace type CORDOBA object, or setup as a
...
%
%    CORDOBA trace data can be in stream type form (sequence of scalars)
%    The data can be indexed by index numbers or by time stamps. CORDOBA
%    stream type traces has a specific data representation and several
%    CORDOBA methods are specialized on trace typed data.
%
%    Copyright(c): Bluenetics 2020 
%
%    See also: CORDOBA, LOG, PLOT, LABEL, CONFIG, CATEGORY, SUBPLOT
%
   while (nargin == 1)                % check nargin
      %o = ~container(o) && isequal(o.type,'trace');
      o = ~container(o) && ~isequal(o.type,'pkg');
      return
   end
%
...
%
   ...
   ok = isa(list{1},'char') && ~o.is(list{1},'bulk');
   if ~ok && (length(list) == 1)
      arg = list{1};
