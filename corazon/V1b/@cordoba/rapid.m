...
%        Carma class.
%
%           rapid(cordoba)             % launch shell
%           rapid(cordoba,'tornado')   % new class name 'espresso'
...
function o = Init(o)                   % Init Shell                    
   ...
   comment = {'Create a new class derived from Cordoba class'};
...
%
%    ooo = mitem(oo,'Stuff');
%    set(mitem(ooo,inf),'enable',onoff(o,{'weird','ball','cube'},'corazon'));
%    oooo = mitem(ooo,'Text File (.txt)',{@ExportCb,'WriteStuffTxt','.txt',@corazon});
%    set(mitem(oooo,inf),'enable',onoff(o,{'pln','smp'},'corazon'));
%
%    ooo = mitem(oo,'$Name');
...
%    oo = cast(oo,'cordoba');            % cast to cordoba object
