...
%
   ...
   obj.tag = mfilename;                   % class name
   format = ['#',upper(obj.tag)];         % format
   par.class = obj.tag;                   % set parameters
   par = parinit(format,par);             % parameter init
   
   she = shell(format,par);               % create a shell object
   obj = class(obj,obj.tag,she);          % derived construction from SHELL
...
function parameter = parinit(format,parameter) % init parameters with defaults
...
% do some format specific special treatments
   ...
   switch format
      ...
      case {'#TOOLBOX','toolbox'}
         title = 'DS Toolbox';  list = {'file','tools','info'}; 
      case {'#ROBO','#TUTORIAL','tutorial'}
         list = {{'tutorial'},'file'};  title = 'ROBO Tutorial';
      otherwise
         title = '';
         list = {'file','view','analyse','measure','calcs','invars',...
                 'footprint','simulation','info'};
   end
