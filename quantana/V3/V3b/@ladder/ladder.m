...
%   
   ...
   path = upath(smart,which('quantana'));
   idx = min(findstr(path,'/@quantana/quantana.m'));
   path(idx:end) = '';
   
   [dat.sphx,dat.sphy,dat.sphz] = sphere(150);
   dat.opcre = smart(imread([path,'/image/opcre.jpg']));
   dat.opani = smart(imread([path,'/image/opani.jpg']));
