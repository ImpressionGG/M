...
%
   ...
   %dPsi = dat.dPsi;             % get pre calculated dpsi table
   ...
   if (any(idx ~= floor(idx) | idx == 0))    % pseudo eigen function
      for (i=length(idx):-1:1)
         ...
         [psi1,ev1,dpsi1] = eig(obj,fidx,t);
         [psi2,ev2,dpsi2] = eig(obj,fidx+1,t);
         psi(:,i) = psi1*(1-lambda) + psi2*lambda;
         %dpsi(:,i) = dpsi1*(1-lambda) + dpsi2*lambda;
   ...
   if (m*n == 0)                         % basis has not been provided
      ...
      %dpsi = dPsi(:,idx);                % eigen function gradiewnts
