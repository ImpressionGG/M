...
%
   func = arg(obj);
   if isempty(func)
      mode = setting('scatter.func');
   else
      setting('scatter.func',func);
   end
...
%
   ...
   smo = timer(smart,dt);                        % setup timer for animation
   ...
   while (~stop(smo))
      for (t = 0:dt:tmax)
         ...
         if (match(dim,{'1D','1C'}))
            smo = update(smo,text(-0.5,0.14,sprintf('t = %3.1f',t)));
         else
            if (mode == 1)
         ...
         smo = wait(smo);   % wait on timer tic
         if (stop(smo))
            break;
         end
      end
