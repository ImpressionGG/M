...
function list = Import(o,package,folder,table) % Import Files of Table 
   ...
   message(o,'Importing Package ...',...
      ...
      % provide package info file, if not existing
      
   ProvidePackageInfo(o,folder);
      ...
      switch suffix
         case '.pkg'
            ...
            elseif (length(packages) > 1)
               message(o,'Error: multiple package info files (.pkg) found!',...
                       'Cannot handle!');
               return
            else
         ...
         otherwise    
            ...
            list = Files(packages{1},list,folder,driver);
...
function ProvidePackageInfo(o,folder)  % Provide PKG info if not exist 
   files = dir(folder);
   for (i=1:length(files))
      file = files(i).name;
      [~,~,ext] = fileparts(['dummy/',file]);
      
      if isequal(ext,'.pkg')
         return                        % PKG info exists, no action!
      end
   end
