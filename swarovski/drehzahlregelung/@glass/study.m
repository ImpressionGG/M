...
function o = Setup(o)                  % Setup Study Menu              
   ...
   oo = mhead(o,'Study');
   ooo = mitem(oo,'Read Data',{@ReadData});
...
%
% STUDY2 Simulation data is stored in a GLASS object of type 'trigo',
...
%
   oo = glass('trigo');                % create a 'trigo' typed object
...
%
% STUDY3 Simulation data is stored in a GLASS object of type 'expo',
%        stored into clipboard for potential paste and plotted. If
%        pasted (with Edit>Paste) object can be further analyzed using
%        'Plot>Stream Plot'.
%        Use 'Plot>Stream Plot X', 'Plot>Stream Plot Y' or 'Plot/Scatter
...
%        while switching signal configurations in View/Signal menu.
%
   oo = glass('expo');                 % create an 'expo' typed object
   oo = log(oo,'t','x','y');           % setup a data log object
...
%
% STUDY4 Repeated simulation data is stored in a GLASS object of type
...
%        while switching signal configurations in View/Signal menu.
%
   oo = glass('trigo');                % create an 'trigo' typed object
   oo = log(oo,'t','x','y');           % setup a data log object
...
function o = ReadData(o)
   ...
   oo = glass('trigo');
   Ts = 0.01;
   oo.par.title = 'Drehzahländerung 1';
   oo.par.Ts = Ts;               % Abtastzeit   

   oo.data.t = (M(:,1)'-1)*Ts;   % Zeit
   oo.data.n = M(:,2)';          % Drehzahl
   oo.data.d2 = M(:,3)';         % Durchmesser oben
   oo.data.d1 = M(:,4)';         % Durchmesser unten
...
% Messdatensatz 2
   ...
   oo = glass('trigo');
   oo.par.title = 'Drehzahländerung 2';
   oo.par.Ts = Ts;               % Abtastzeit   
