...
function o = Init(o)                   % Init Object                   
   ...
   o = set(o,{'title'},'Glass Shell');
   o = set(o,{'comment'},{'Playing around with GLASS objects'});
...
function oo = New(o)                   % New Menu Items                
   ...
   ooo = mitem(oo,'Glass');
   oooo = mitem(ooo,'New Plain Glass',{@NewCb,'Plain','pln'});
   oooo = mitem(ooo,'New Simple Glass',{@NewCb,'Simple','smp'});
   plugin(oo,'glass/shell/New');
   return
...
function oo = Import(o)                % Import Menu Items             
   ...
   ooo = mitem(oo,'Glass');
   oooo = mitem(ooo,'Glass Data (.log)',{@ImportCb,'ReadGlassLog','.log',@glass});
   oooo = mitem(ooo,'Glass Data (.dat)',{@ImportCb,'ReadGlassDat','.dat',@glass});
   plugin(oo,'glass/shell/Import');
   return
...
function oo = Export(o)                % Export Menu Items             
   ...
   ooo = mitem(oo,'Glass');
   set(mitem(ooo,inf),'enable',onoff(o,{'log','pln','smp'},'glass'));
   oooo = mitem(ooo,'Glass Log Data (.log)',{@ExportCb,'WriteGlassLog','.log',@glass});
   set(mitem(oooo,inf),'enable',onoff(o,'log','glass'));
   oooo = mitem(ooo,'Plain Glass (.dat)',{@ExportCb,'WriteGlassDat','.dat',@glass});
   set(mitem(oooo,inf),'enable',onoff(o,'pln','glass'));
   oooo = mitem(ooo,'Simple Glass (.dat)',{@ExportCb,'WriteGlassDat','.dat',@glass});
   set(mitem(oooo,inf),'enable',onoff(o,'smp','glass'));
   
   plugin(oo,'glass/shell/Export');  % plug point
   return
...
function oo = Collect(o)               % Configure Collection          
   ...
   plugin(o,'glass/shell/Collect');   % plug point
   
   if isequal(class(o),'caramel')
      table = {{@read,'carabao','ReadStuffTxt','.txt'}};
      collect(o,{'weird','ball','cube'},table); 
   end
   if isequal(class(o),'glass')
      table = {{@read,'glass','ReadGlassDat','.dat'}};
      collect(o,{'pln','smp'},table); 
   end
   if isequal(class(o),'glass')
      table = {{@read,'glass','ReadGlassLog','.log'}};
      collect(o,{'log'},table); 
   end
   if isequal(class(o),'glass')
      table = {{@read,'caramel','ReadPkgPkg','.pkg'},...
               {@read,'glass','ReadGlassDat','.dat'}};
      collect(o,{'pln','smp'},table); 
   end
   oo = o;
end
...
%
   ...
   plugin(oo,'glass/shell/Signal');   % plug point
end
...
function oo = Analysis(o)              % Analysis Menu                 
   ...
   plugin(oo,'glass/shell/Analysis');
end
...
function oo = Info(o)                  % Info Menu                   
   ...
   oooo = mitem(ooo,['Glass Class: Version ',version(glass)]);
   ooooo = mitem(oooo,'Edit Release Notes','edit glass/version');
   plugin(oo,'glass/shell/Info');
end
...
function oo = ReadGlassDat(o)          % Read Driver for glass .dat  
   path = arg(o,1);
   oo = read(o,'ReadGenDat',path);
   oo = Config(oo);                    % overwrite configuration
end
function oo = WriteGlassDat(o)         % Read Driver for glass .dat  
   path = arg(o,1);
