...
function oo = Menu(o)                  % Setup Study Menu              
   ...
   ooo = mitem(oo,'Package List',{@WithCuo,'PackageList'},[]);
   ...
   setting(o,{'ellipse.phi'},10);
   
   oo = mitem(o,'-');
   oo = mitem(o,'Ellipse');
   ooo = mitem(oo,'Ellipse Study 1',{@WithCuo,'EllipseStudy1'},[]);
   ooo = mitem(oo,'Ellipse Study 2',{@WithCuo,'EllipseStudy2'},[]);
   ooo = mitem(oo,'-');
   ooo = mitem(oo,'Phi',{},'ellipse.phi');
   choice(ooo,[0:10:180],{});
end
...
function o = PackageList(o)
   if ~type(o,{'pkg'})
      message(o,'No package!');
      return
   end
   
   par = o.par;
   issue = false;
   for (i=1:length(par.files))
      number = par.number(i);
      station = par.station{i};
      file = par.files{i};
      [dir,name,ext] = fileparts(file);
      
      idx = find(file=='_');
      
      s = file(idx(2)+1:idx(3)-1);
      n = name(idx(6)+1:end);
      n = sscanf(n,'%f');
      
      mismatch = (n ~= number) || ~isequal(upper(s),upper(station));
      issue = issue || mismatch;
      
      star = o.iif(mismatch,'*',' ');
      
      fprintf('%s %5g: %6s  %s\n',star,number,station,file);
   end
   
   if (issue)
      message(o,'package parameters printed to console!',{'some issues!'});
   else
      message(o,'package parameters printed to console!',{'no issues!'});
   end
...
function o = EllipseStudy1(o)          % Ellipse Study #1              
   X = [5*randn(1,1000); 1*randn(1,1000)];   
   phi = opt(o,{'ellipse.phi',15})*pi/180;
   ...
   subplot(o,[2 1 1]);
   plot(o,Y(1,:),Y(2,:),'rrmo2');
   hold on;
   %plot(o,[0 v1(1)]*s(1),[0 v1(2)]*s(1),'r3');
   %plot(o,[0 v2(1)]*s(2),[0 v2(2)]*s(2),'bc3');
   %dark(o);
   ...
   hold on;
   plot(o, 3*sigx*cos(p), 3*sigy*sin(p), 'r3');
   
   subplot(o,[2 1 1]);
   YY = TT'*[3*sigx*cos(p); 3*sigy*sin(p)];
   plot(o, YY(1,:), YY(2,:), 'bc3');
end
function o = EllipseStudy2(o)          % Ellipse Study #2  
   n = 10000;
   X = [5*randn(1,n); 1*randn(1,n)];   
   phi = opt(o,{'ellipse.phi',15})*pi/180;
   T = [cos(phi),-sin(phi);sin(phi) cos(phi)];
   Y = T*X;
   x = Y(1,:);  y = Y(2,:);
   ...
   if opt(o,{'view.ellipse',1})
      hold on
      plot(o,X,Y,'r3');
   end
   
   title(sprintf('Phi = %g°',o.rd(phi,0)));
   set(gca,'DataAspectRatio',[1 1 1]);
end
