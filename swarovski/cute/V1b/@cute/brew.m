...
%            oo = brew(o,'Cutoff')     % calculate cutoff frequency
%            oo = brew(o,'Metrics')    % calculate metrics
% 
...
function oo = Brew(o)                  % Brew All (If Not Yet Done)    
   ...
   if isempty(bag)                     % if brew segment not up-to-date
      Message(o,'brewing data ...',...
                {get(o,{'title',''}),['oid: ',get(o,{'oid',''})]});
      pause(0.1);
      
      oo = Basis(o);                   % provide cache basis data
      oo = Cluster(oo);                % estimate cluster indices and time
      oo = Rotate(oo);
      oo = Cutoff(oo);                 % calculate cutoff frequency
      oo = Metrics(oo);                % calculate metrics data
      
         % brewing plug point
      
      ooo = plugin(oo,'cute/brew/Brew'); 
      if isobject(ooo)                 % only if cache has been served
         oo = arg(ooo,{});             % copy with cleaned args
      end      
   else
...
%==========================================================================
% Cutoff Frequency & Metrics Data
%==========================================================================

function oo = Cutoff(o)                % Cutoff Frequency              
   kfcut = opt(o,{'thdr.cutoff',1.5});
   ...
   assert(all(f1==f2) && all(f2==f3));

      % calculate geometric mean of FFTs of each component
      
   X = sqrt(X1.*X1 + X2.*X2 + X3.*X3);   % geometric mean
   ...
   oo = cache(o,'brew.fcut',fcut);
end
function oo = Metrics(o)               % Metrics Data                  
   if ~type(o,{'cut'})
      oo = o;
      return
   end
   
   [cpks,cpkw,mgrs,mgrw,mgn,sig] = cpk(o,'a');
   thd = thdr(o,'a');                  % total harmonic distortion
   ...
   oo = cache(o,'brew',bag);           % store bag in 'brew' cache segment
end
...
%
   ...
   if (control(oo,'verbose') >= 3)
      fprintf('brew/Package %s\n',get(oo,{'oid','?'}));
   end
   ...
   ooo = plugin(oo,'cute/brew/Package'); 
   if isobject(ooo)                    % only if cache has been served
      oo = arg(ooo,{});                % copy with cleaned args
   end
...
function o = PackageBrew(o,list)       % Brew Package Data             
   if (control(o,'verbose') >= 3)
      fprintf('   brew/PackageBrew %s\n',get(o,{'oid','?'}));
   end
   
   assert(isequal(o.type,'pkg'));
   
   files = {};  objects = {};  objids = {};
   cpks = [];  cpkw = [];  mgrs = [];  mgrw = [];  
   mgn = [];   sig = [];
   
      % build a sorted list of available object numbers
   
   numbers = [];  ndx = [];
   for (i=1:length(list))
      oo = list{i};
      numb = get(oo,{'number',0});
      if (numb > 0) && type(oo,{'cut'})
         numbers(end+1) = numb;
         ndx(end+1) = i;
      end
   end
   [numbers,sdx] = sort(numbers);
   nlist = list(sdx);                  % nlist contains only data objects
   ...
   for (i=1:n)
      ...
      numbers(i) = get(oo,{'number',0});   % be safe - overwrite!
      objects{end+1} = get(oo,{'title',''});
      objids{end+1} = id(oo);
      files{end+1} = get(oo,{'file',''});
      
      msg = sprintf('Cache refreshing %d of %d: %s',i,n,objects{end});
      progress(o,msg,(i-1)*100/n);
      oo = opt(oo,'progress',false);   % dont show progress by helpers
   ...
   o = set(o,'objids',objids);
   o = set(o,'files',files);
   
   o = data(o,'numbers,facettes,cpks,cpkw,mgrs,mgrw,mgn,sig',...
               numbers,facettes,cpks,cpkw,mgrs,mgrw,mgn,sig);
   o = data(o,'thdr',thd);
end
function o = PackageRefresh(o)         % Soft Refresh Package Cache    
   bag.objects = get(o,{'objects',{}});
   bag.objids = get(o,{'objids',{}});
   bag.files = get(o,{'files',{}});
   
   bag = RefreshPar(o,bag,'project',{});
   bag = RefreshPar(o,bag,'machine',{});
   bag = RefreshPar(o,bag,'package',{});
   bag = RefreshPar(o,bag,'number',{});
   bag = RefreshPar(o,bag,'date',{});
   bag = RefreshPar(o,bag,'time',{});

   bag = RefreshPar(o,bag,'station',{});
   bag = RefreshPar(o,bag,'apparatus',{});
   bag = RefreshPar(o,bag,'damping',{});
   bag = RefreshPar(o,bag,'kappl',{});
   bag = RefreshPar(o,bag,'lage',[]);
   bag = RefreshPar(o,bag,'vcut',[]);
   bag = RefreshPar(o,bag,'vseek',[]);
      
   bag = RefreshData(o,bag,'numbers',[]);
   bag = RefreshData(o,bag,'facettes',[]);
   bag = RefreshData(o,bag,'cpks',[]);
   bag = RefreshData(o,bag,'cpkw',[]);
   bag = RefreshData(o,bag,'mgrs',[]);
   bag = RefreshData(o,bag,'mgrw',[]);
   bag = RefreshData(o,bag,'mgn',[]);
   bag = RefreshData(o,bag,'sig',[]);
   bag = RefreshData(o,bag,'thdr',[]);

   USL = opt(o,{'spec.thdr',0.05});
   bag.chk = USL./bag.thdr;
   o = cache(o,'package',bag);         % store bag in cache
   
   function bag = RefreshPar(o,bag,tag,default)
      value = get(o,{tag,default});
      bag.(tag) = value;
   end
   function bag = RefreshData(o,bag,tag,default)
...
function o = Message(o,title,comment)  % Show Message & Wait a Bit     
   if ~opt(o,{'verbose',0})
      msg = title;
      if iscell(comment)
         msg = sprintf('%s (%s)',title,comment{1});
      end
