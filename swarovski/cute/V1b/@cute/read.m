...
%
   [gamma,oo] = manage(o,varargin,@Error,@ReadPkgPkg,...
                       @ReadCutCsv,@ReadScpCsv,...
...
%
% READMPL  Measurement plans contain the full information of the .xlsx 
%          measurement plan. 
   o = opt(o,'full',true);
   oo = ReadPkgXlsx(o);                % delegate to ReadPkgXlsx
   oo.type = 'mpl';                    % use 'mpl' type (otherwise issues!)
   oo.par.package = [];
   oo.par.title = ['Measurement Plan ',get(oo,{'date',''})];
end
function oo = ReadPkgXlsx(o)           % Read Measurement Plan into Pkg
   ...
   progress(o,['Importing ',file,ext],1);
   
      % read .CSV file into table, extract names and table matrix (M)
      % and store as object (working) variables. Note that MATLAB 
      % has different behavior compared to MATLAB R2019b. Thus we try
      % reading MATLAB R2020 foprmat, if this fails we repeat in MATLAB
      % R2019b style
      
   try                                 % try MATLAB R2020 style
      T = readtable(path,'PreserveVariableNames',true,'Format','auto');
   catch                               % otherwise try MATLAB R2019b style
   ...
   progress(o,['Importing ',file,ext],10);
   ...
   for (i=1:length(files))
      ...
      files{i} = filename;             % add extension to file name
   ...
   function o = PackageInfo(o,path)    % Extract Package Info          
      ...
      folder = [folder,fext];          % reconstruct folder name
         ...
         % extract run number digits and machine name from folder name

      oid = id(o,folder);              % construct object ID
      idx = find(oid =='.');
      machine = oid(2:idx(1)-1);
      package = oid(1:idx(2)-1);       % package name
      digits = oid(idx(1)+1:idx(2)-1);     
      
         % store parameters
         
      o.par.package = package;
      o.par.oid = oid;
      o.par.date = ['20',digits(1:2),'-',digits(3:4),'-',digits(5:6)];
      o.par.dir = dir;
      o.par.file = [package,'.CUT.pkg'];
      o.par.title = ['Package Info ',oid];
...
function oo = ReadCutCsv(o)            % Read Driver for .csv File     
%
% READCUTCSV
%
%     Options:
%
%        upgrade:   auto upgrade package info 8default: 1)
%
   path = arg(o,1);                    % get path arg
   [dir,file,ext] = fileparts(path);
   
      % file name must not start with a 'Scope_' prefix
      
   if isequal(strfind(file,'Scope_'),1)
      message(o,'Bad file name for CUT object!',...
              {['File name: ',file],...
               'Prefix ''Scope_'' not allowed for CUT objects!'});
      oo = [];
      return
   end
   
      % extract package ID, machine ID and run number ...
      
   try
      [~,package,pxt] = fileparts(dir);
      folder = [package,pxt];    
      [oid,~,machine] = id(o,folder);  % extract preliminary oid, etc.
      package = oid(1:end-6);
      
      idx = find(file=='_');
      numbtxt = file(idx(end)+1:end);
      number = sscanf(numbtxt,'%f');   % extract run number
      
      oid = sprintf('%s.%d.CUT',package,number);  % finalize oid
   catch
      oid = '';  package = '';  machine = ''; number = 0;
   end
   ...
   oo.par.package = package;           % package name
   oo.par.oid = oid;                   % object ID
   ...
   for (i=1:length(names))
      ...
      % upgrade data from package & article object
      
   if opt(o,{'upgrade',true})
      oo = upgrade(o,oo,o.data);       % upgrade data  object
   end
end
function oo = ReadScpCsv(o)            % Read Driver for .csv File     
%
% READSCPCSV
%
%     Options:
...
%
   ...
   [dir,file,ext] = fileparts(path);
      ...
      % scope foles must begin with prefix 'Scope_'
      
   if ~isequal(strfind(file,'Scope_'),1)
      message(o,'Bad file name for SCP object!',...
         {['File name: ',file],...
         'Expect prefix ''Scope_'' for scope data files!'});
      oo = [];
      return
   end
   ...
   try
      ...
      oid = sprintf('%s.%d.SCP',package,number);  % finalize oid
   catch
      oid = '';  package = '';  machine = ''; number = 0;
   end
   ...
   oo = cute('scp');                   % create CUTE object of type 'SCP'
   oo = figure(oo,figure(o));          % copy figure
   oo.par.title = file;
