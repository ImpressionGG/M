function [package,typ,name,run,mach,station,kappl,vcut,vseek] = split(o,title)
%
...
%  if isempty(mach) || (any((mach < '0') | (mach > '9')))
   ...
   txt = name;
   [txt,station] = Extract(o,name,'station');
   [txt,kappl] = Extract(o,txt,'kappl');
   [txt,vcut] = Extract(o,txt,'vc');
   [txt,vseek] = Extract(o,txt,'vs');
   
   package = sprintf('@%s.%d',mach,run);
end
function [txt,value] = Extract(o,txt,tag)  % extract comma seperated args
   value = '';
   idx = o.either(find(txt==','),length(txt)+1);
   value = txt(1:idx(1)-1);
   txt = txt(idx(1)+1:end);

   idx = find(value == '=');
   if o.is(idx)
      jdx = strfind(value,[tag,'=']);
      if o.is(jdx)
         value = sscanf(value(length(tag)+2:end),'%f');
      else
