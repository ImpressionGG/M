...
function oo = Brew(o)                  % Brew All (If Not Yet Done)    
   ...
   if isempty(bag)                     % if brew segment not up-to-date
      Message(o,'brewing data ...',{get(o,{'title',''})});
      pause(0.1);
...
function o = PackageBrew(o,list)       % Brew Package Data             
   ...
   n = length(list);
   for (i=1:n)
      oo = Seek(o,list,i);             % find obj with number i
      if isempty(oo)
         error('cannot brew package');
      end
   ...
   function oo = Seek(o,list,i);       % Find Object By Number         
      for (j=1:length(list))
         oo = list{j};
         number = get(oo,{'number',-1});
         if (number == i)
            return                     % found - bye!
         end
      end
