...
%           a1 = cook(o,'a1');
%           a2 = cook(o,'a2')
%           oo = opt(o,'range',[10000 20000]);
%           diagram(oo,'A12',a1,a2,[2 2 1],'');
%
%        Options:
%
%           metrics:         show metrics on title (default false)
%           range:           plot only range of x and y
%           angle:           display ellipse angle in heading (default 1)
%
...
%
   ...
   function o = Dispatch(o,mode)       % Dispatch Plot Parameters      
      ...
      switch mode
         ...
         case 'V2'
            tit = 'Cross Velocity 2';  ylab = 'v2 [mm/s]';  col = 'b';
            xyz = 0;  kind = 'v';  cat = 1;  specs = opt(o,{'specs.v',[]});
         case 'V3'
         ...
         case 'S2'
            tit = 'Cross Elongation 2';  ylab = 's2 [um]';  col = 'g';
            xyz = 0;  kind = 's';  cat = 5;  specs = opt(o,{'specs.s',[]});
         case 'S3'
   ...
   function Plot(o)                    % Actual Plotting               
      ...
      subplot(sub(1),sub(2),sub(3));
         ...
         % store key data in axes' shelf (this must be before replacing
         % x and y by range selection)
      ...
      if (xyz && opt(o,{'view.ellipse',0}))
         col = o.iif(dark(o),'w1','k1');
         
         [phi,X,Y] = ellipse(o,x,y);
         plot(o,X,Y,col);
      end
      ...
      if (xyz)
         bag.closeup = false;
         shelf(o,gca,'',bag);          % store bag in axes shelf
      end
   end
   function Labels(o)                  % Provide Labels                
      metrics = opt(o,{'metrics',0});
      ...
      if ~isempty(epilog)
         title([tit,' ',epilog]);
      elseif (xyz && opt(o,{'view.ellipse',0}) && opt(o,{'angle',1}))
         title(sprintf('%s (%g°)',tit,o.rd(phi,0)));
      else
         title(tit);
      end
   end
