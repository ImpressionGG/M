function [cpks,cpkw,mgrs,mgrw,mgn,sig,avg] = cpk(o,sym)
%
...
%
%          [cpks,cpkw,mgrs,mgrw,mgn,sig,avg] = cpk(o,sym)
%
%       Examples:
%
...
%
%          mgr = cook(o,'mgr')         % easiest way to get proper Mgr
%          cpk = cook(o,'cpk')         % easiest way to get proper Cpk
%
...
%
   ...
   if (facette > 0)
      [cpks,cpkw,mgrs,mgrw,mgn,sig,avg] = Cpk(o,sym,1,facette,lsl,usl);
      [thd,chk] = thdr(o);
   else
      if (facette < 0)
         nf = -nf;
      ...
      for (k=1:abs(nf)) 
         [cpks(k),cpkw(k),mgrs(k),mgrw(k),mgn(k),sig(k),avg(k)] = ...
   ...
   function Show5(o)                   % Show FinalPart of Graphics    
      if (show)
         ...
         tit = sprintf(['Magnitude: %g,  Mgr: %g (%g),  Cpk: %g (%g)',...
                  ' - Threshold %g%%'], o.rd(mgn,1), o.rd(mgrs,2),...
                  o.rd(mgrw,2), o.rd(cpks,2), o.rd(cpkw,2),(1-pareto)*100);
         title(tit);
         ylabel(sym);
         xlabel('time');
