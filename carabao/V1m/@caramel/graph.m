...
%    Example:
% 
%       o = config(pull(caramel),[]);  % set all sublots to zero
...
%  end
   ...
   %clip(o,o);
      ...
      % we cast object completely to a CARAMEL object before we put
      % it into the clipboard. This allows to leverage the dynamic 
      % plot menu of CARAMEL objects
   
   oc = cast(oo,'caramel');
   oc.tag = 'caramel';                 % complete conversion to caramel
   clip(oc,oo);
