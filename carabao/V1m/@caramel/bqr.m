...
function oo = Import(o)                % Import Menu Items             
   ...
   ooo = mitem(oo,'Data File (.dat)',{@ImportCb,'ReadBqrDat','.dat',@caramel});
   ooo = mitem(oo,'M-File (.m)',{@ImportCb,'ReadBqrM','.m',@caramel});
   return
...
function oo = Export(o)                % Export Menu Items             
   ...
   ooo = mitem(oo,'Data File (.dat)',{@ExportCb,'WriteBqrDat','.dat',@caramel});
   return
...
function oo = Collect(o)               % Configure Collection          
   table = {{@read,'caramel','ReadPkgPkg','.pkg'},...
            {@bqr,'caramel','ReadBqrDat','.dat'},...
            {@bqr,'caramel','ReadBqrM', '.m'}};
...
%
% from here 'oo' will be a newly constructed BQR-type CARAMEL object
%
   oo = opt(caramel('bqr'),opt(o));
...
function oo = ReadBqrM(o)              % read BQR object from .m file  
   ...
   if ~isempty(odat)
      ...
      for (i=1:length(odat.data))
         ...
         oo.par.version = ['CARABAO ',version(carabao)];
      ...
      oo = caramel('bqr');             % create a container object
...
%
   if exist('danaobj') ~= 2
      caramelpath = which('caramel');
      [dir,file,ext] = fileparts(caramelpath);
