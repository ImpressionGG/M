...
%
%         See also: CARAMEL, SAMPLE, BASIS, PBI, DANA, KEFALON
%
   if (nargin == 0)
...
function o = Import(o)                 % Add Import Menu               
   oo = mitem(o,'TCB');
   ooo = mitem(oo,'TCB Trace (.trace)',{@ImportCb,@ReadTcbTrace,'.trace',@caramel});
   ooo = mitem(oo,'TCB Trace (.dat)',{@ImportCb,@ReadTcbDat,'.dat',@caramel});
   return
...
function o = Export(o)                 % Add Export Menu               
   ...
   ooo = mitem(oo,'TCB Trace (.dat)',{@ExportCb,'WriteTcbDat','.dat',@caramel});
   return
