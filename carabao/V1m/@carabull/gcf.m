...
%
%       gcf(caracow,hGui)    % register GUI handle
%
...
%          gcf(caracow,handles.figure1);     % register GUI figure handle
%            :
%       end
%
...
%          try          
%             gcf(caracow,handles.figure1);    % register GUI figure handle
%          catch             % avoid a crash if CARACOW is not available
%          end               % try/catch allows to run GUI without CARACOW
%           :
%       end
%
%    Code lines: 40
%
%    See also: CARABULL, FIGURE, PUSH, PULL
%
   if (nargin == 1)
