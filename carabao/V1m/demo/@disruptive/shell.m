...
function o = Init(o)                   % Init Object                   
   ...
   o = set(o,{'title'},'Disruptive Shell');
   o = set(o,{'comment'},{'Playing around with DISRUPTIVE objects'});
...
function oo = New(o)                   % New Menu Items                
   ...
   ooo = mitem(oo,'Disruptive');
   oooo = mitem(ooo,'New Plain Disruptive',{@NewCb,'Plain','pln'});
   oooo = mitem(ooo,'New Simple Disruptive',{@NewCb,'Simple','smp'});
   plugin(oo,'disruptive/shell/New');
   return
...
function oo = Import(o)                % Import Menu Items             
   ...
   ooo = mitem(oo,'Disruptive');
   oooo = mitem(ooo,'Disruptive Data (.log)',{@ImportCb,'ReadDisruptiveLog','.log',@disruptive});
   oooo = mitem(ooo,'Disruptive Data (.dat)',{@ImportCb,'ReadDisruptiveDat','.dat',@disruptive});
   plugin(oo,'disruptive/shell/Import');
   return
...
function oo = Export(o)                % Export Menu Items             
   ...
   ooo = mitem(oo,'Disruptive');
   set(mitem(ooo,inf),'enable',onoff(o,{'log','pln','smp'},'disruptive'));
   oooo = mitem(ooo,'Disruptive Log Data (.log)',{@ExportCb,'WriteDisruptiveLog','.log',@disruptive});
   set(mitem(oooo,inf),'enable',onoff(o,'log','disruptive'));
   oooo = mitem(ooo,'Plain Disruptive (.dat)',{@ExportCb,'WriteDisruptiveDat','.dat',@disruptive});
   set(mitem(oooo,inf),'enable',onoff(o,'pln','disruptive'));
   oooo = mitem(ooo,'Simple Disruptive (.dat)',{@ExportCb,'WriteDisruptiveDat','.dat',@disruptive});
   set(mitem(oooo,inf),'enable',onoff(o,'smp','disruptive'));
   
   plugin(oo,'disruptive/shell/Export');  % plug point
   return
...
function oo = Collect(o)               % Configure Collection          
   ...
   plugin(o,'disruptive/shell/Collect');   % plug point
   
   if isequal(class(o),'caramel')
      table = {{@read,'carabao','ReadStuffTxt','.txt'}};
      collect(o,{'weird','ball','cube'},table); 
   end
   if isequal(class(o),'disruptive')
      table = {{@read,'disruptive','ReadDisruptiveDat','.dat'}};
      collect(o,{'pln','smp'},table); 
   end
   if isequal(class(o),'disruptive')
      table = {{@read,'disruptive','ReadDisruptiveLog','.log'}};
      collect(o,{'log'},table); 
   end
   if isequal(class(o),'disruptive')
      table = {{@read,'caramel','ReadPkgPkg','.pkg'},...
               {@read,'disruptive','ReadDisruptiveDat','.dat'}};
      collect(o,{'pln','smp'},table); 
   end
   oo = o;
end
...
%
   ...
   plugin(oo,'disruptive/shell/Signal');   % plug point
end
...
function oo = Plot(o)                  % Plot Menu                     
   ...
   plugin(oo,'disruptive/shell/Plot');
end
...
function oo = Analysis(o)              % Analysis Menu                 
   ...
   plugin(oo,'disruptive/shell/Analysis');
end
...
function oo = Study(o)                 % Study Menu                  
   ...
   plugin(oo,'disruptive/shell/Study');
end
...
function oo = Info(o)                  % Info Menu                   
   ...
   oooo = mitem(ooo,['Disruptive Class: Version ',version(disruptive)]);
   ooooo = mitem(oooo,'Edit Release Notes','edit disruptive/version');
   plugin(oo,'disruptive/shell/Info');
end
...
function oo = ReadDisruptiveDat(o)          % Read Driver for disruptive .dat  
   path = arg(o,1);
   oo = read(o,'ReadGenDat',path);
   oo = Config(oo);                    % overwrite configuration
end
function oo = WriteDisruptiveDat(o)         % Read Driver for disruptive .dat  
   path = arg(o,1);
