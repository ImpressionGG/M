function [avg,sig] = analysis(kind,type,N,Tf,Kp)
%
% ANALYSIS        Analyse Cpk distribution of filter settings
%
%                    alysis(kind,type,N,Tf,Kp,ref)
%                    alysis(kind,[1 det ref],N,Tf,Kp,ref)
%
%                 1st arg 'kind' is a string describing the kind of filter.
%                 2nd arg 'type' is either an integer number or a cell aray:
%                 {type,det,ref}
%
   Nx  = eval('Nx','0');
   N  = eval('N','5');
   ...
   for (i=1:length(Kp))
       ...
       for (j = 1:length(Tf))
          ...
          for (k=1:kmax)   % for (k = 1:N)
             ...
             [Cpk,t,r,w,y,tf,f,Kind] = simu(kind,Type,Tf(j),Kp(i),N/kmax);
             cpk = [cpk,Cpk];
   ...
   subplot(221);
   delete(H);
   [cpk,t,r,w,y,tf,f] = simu(kind,Type,Tf(jopt),Kp(iopt));
   hold off;
   plot(t,0*t,'k');  hold on;
   for (k=1:length(t))
