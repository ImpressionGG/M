function [out1,out2] = kafi(arg1,arg2,arg3) % Kalman Filter            
%
...
%              F = kafi(2);   % double integrator model (DKAFI)
%              F = kafi(3);   % scalar Kalman filter (SKAFI)
%              F = kafi(4);   % twin Kalman filter (TKAFI)
%              F = kafi(5);   % order #0 polynomial Kalman filter (PKAFI0)
%              F = kafi(6);   % order #1 polynomial Kalman filter (PKAFI1)
%
...
% sates 'x1' and 'x2' to the well defined values zero!
   ...
   if (nargin == 3) % reset filter
      F = arg1;  u  = arg2;  t = arg3;
      if (F.Kp < 0)  % this is for easier simulation
         [out1,out2] = Operate0(arg1,arg2,arg3);
      else
         switch F.type
         case 0
            [out1,out2] = Operate0(F,u,t);
         case 1
            [out1,out2] = Operate1(F,u,t);
         case 2
            [out1,out2] = Operate1(F,u,t);
         case 3
            [out1,out2] = Operate3(F,u,t);
         case 4
            [out1,out2] = Operate4(F,u,t);
         case 6
            [out1,out2] = Operate6(F,u,t);
         otherwise
...
function F = Reset(F)                  % Reset Kalman Filter           
%
...
function [y,F] = Operate0(F,u,t)       % Ordinary PT1 Filter           
%
...
function [y,F] = Operate1(F,u,t)       % DI Kalman Filter (DKAFI)      
%
...
function [y,F] = Operate3(F,u,t)       % Scalar Kalman Filter (SKAFI)  
%
...
function [y,F] = Operate4(F,u,t)       % Twin Kalman Filter (TKAFI)    
%
...
function [y,F] = Operate6(F,u,t)       % Polynom Kalman Filter (PKAFI1)       
%
% OPERATE6  Kalman filter operation, type = 5: 
%           Order #1 polynomial Kalman filter
%
%              y = Operate6(F,u,t)         % u: system output
%              y = Operate6(F,{u,c},t)     % c: system (control) input
...
%  H = N * C*P*C';                     % Kalman output gain
   ...
   M = A*P*A' + Q;                     % short hand
   K = M*C' / (C*M*C' + R);            % Kalman gain
   P = (I - K*C) * M;                  % recursion for eror covariance
...
%  x = A*x + B*c + K*(u - C*x);   
   ...
   h = A*x + B*c;                      % half way observer state transition    
   x = h + K*(u - C*h);                % state transition
   y = C*x;                            % observer output
   
      % now we have to calculate the transition of the error covariance 
          
%  P = A*P*A' + Q - K*C*P*A';   

   F.A = A;  F.B = B;  F.C = C;  F.P = P;  F.K = K;  F.M = M;
   F.x = x;  F.y = y;  F.u = u;  F.c = c;  F.h = h;
   F.p = norm(P);
end
...
%
   ...
   F.T = eval('F.T','2');              % general filter time const default
   F.Tcrit = eval('F.T','2');          % critical time for reset
   F.N = eval('F.N','1');              % number of simultaneous tests
   F.Kp = eval('F.Kp','0.1');
   
   switch F.type
      case 1   % F = kafi(1);          % order#2 polynomial Kalman filter
         F.kind = 'dkafi';
      ...
      case 6 % F = kafi(6);            % polynomial Kalman filter (order 1)
         F.kind = 'pkafi1';
         F.Ts = eval('F.Ts','0.2');    % default filter time
         F.Tf = eval('F.Tf','200');    % default filter time
         F.P0 = diag([25000/900, 100]);% covariance of initial state
