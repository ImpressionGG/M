...
%        [yf2k,F2] = eval(cmd);      % Filter F2 operation
   ...
   oo = log(caramel('fito'),'t,r[µ]:k#1,y[µ]:g#1,q[µ]#2:m,f0[µ]#1:b,f1[µ]#1:r,f2[µ]:k#1,n[µ]:g#2,e0[µ]:b#2,e1[µ]:r#2,e2[µ]:k#2');
   t0 = 0;  n = y-r*ones(1,Ns);        % noise
   for (i=1:Ns)
      oo = log(oo);
      f0i = yf0(:,i);  f1i = yf1(:,i);  f2i = yf2(:,i); 
      ni = n(:,i)';  e0i = f0(:,i);   e1i = f1(:,i);   e2i = f2(:,i); 
      oo = log(oo,t0+t',r',y(:,i)',q',f0i',f1i',f2i',ni,e0i',e1i',e2i');
      t0 = t0 + max(t);
   end
   plot(oo,'Put');
...
% now plot results
   ...
   if (nargout > 0)
      ...
      ylabel(sprintf(['ORFI(0): blue,  TWIN(1): red,  ',upper(kind),'(%g): black'],type));
      set(gca,'ylim',[-4,4]);
      shg
   end
