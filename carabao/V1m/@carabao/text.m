...
%
%           smo = opt(carabao,'position','home','append',1);
%           text(smo,'1st line\n2nd line\n3rd line);
%
...
% Push  Push text data
%
   position = either(opt(o,'position'),inf);  % inf means not defined
   size = either(opt(o,'size'),-1);           % -1 means not defined
   
   opti = either(gao('text.options'),[]);
   
   if isempty(opti)
      oo = Options(carabao,[]);                  % init all options
      opti = opt(oo);
   end
...
%
%             text(opt(carabao,'size',8));
%
   size = either(opt(o,'size'),4);
   mode = either(opt(o,'mode'),'all');
