...
%
   ...
   if isequal(sym,':')                 % if symbol denotes time      
      ...
      filtermode = either(opt(o,'filter.mode'),'raw');
      switch filtermode
         case 'raw'
            df = [];
         case 'filter'
...
function d = CookTime(o,d,sym)         % Data Cooking for Time Symbol  
   ...
   switch mode
      ...
      case {'overlay','offsets','sigma','repeatability',...
         ...
         d = 1:size(d,2);           % use position indices instead
...
function d = CookOther(o,d,sym)        % Data Cooking for Other Symbols
   [iif,is] = util(o,'iif','is');      % need some utility
   ...
   while ~is(sym,':')                  % if other symbol than time     
      switch mode
...
%           end
            ...
            elseif is(bias,'deviation')
               d = d - mean(d(:));                % convert to deviation
            end
            
         case 'sigma'                             % std. dev. of columns
            d = iif(size(d,1)==1,0*d,std(d));

         case {'repeatability'}
            avg = iif(size(d,1)==1,d,mean(d));    % row average values
         ...
         case {'average'}                         % mean values of rows
            ...
            d = d';                               % transpose back   
            d = iif(size(d,2)==1,d',mean(d'));    % row average values      
         ...
         case {'spread'}                          % sigma values of rows
            ...
            d = d';                               % transpose back   
            d = iif(size(d,2)==1,0*d',3*std(d')); % row average values      
...
%
   ...
   switch mode
      case {'overlay','offsets','ensemble','average','spread','deviation',...
         ...
         while is(d)                   % previously: while (1)
            D = Pick(o,d)';
            d = reshape(d(1:mn*r),mn,r)'; % transpose after reshaping
            if is(scope)
               d = d';                    % first transpose back
               d = d(:,scope)';           % pick only columns in scope
            elseif is(ignore)
               d(1:ignore,:) = [];        % ignore a number of rows
            end
            assert(isequal(d,D));
            break
         end
...
%
   ...
   if is(scope)
      D = D(:,scope);            % pick only columns in scope
   elseif is(ignore)
      D(:,1:ignore) = [];        % ignore a number of columns
   end
...
%
   either = @o.either;                 % need some utilities
   is = @o.is;                         % need some utilities
...
%
   meth = either(get(o,'method'),'TopLeftColSawtooth');
...
%
   ...
   while (1)
      ...
      if is(scope)
         d1 = d1(:,scope);             % pick only columns in scope
      elseif is(ignore)
   ...
   while (1)
      ...
      if is(scope)
         d2 = d2(:,scope);             % pick only columns in scope
      elseif is(ignore)
...
%
   if is(symbol,symbols{1})
      c = c1 - mean(c1')'*ones(1,r);
   elseif is(symbol,symbols{2})
