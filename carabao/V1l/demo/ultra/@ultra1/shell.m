...
function o = Init(o)                   % Init Object                   
   ...
   o = set(o,{'title'},'Ultra1 Shell');
   o = set(o,{'comment'},{'Playing around with ULTRA1 objects'});
...
function oo = New(o)                   % New Menu Items                
   ...
   ooo = mitem(oo,'Ultra1');
   oooo = mitem(ooo,'New Plain Ultra1',{@NewCb,'Plain','pln'});
   oooo = mitem(ooo,'New Simple Ultra1',{@NewCb,'Simple','smp'});
   plugin(oo,'ultra1/shell/New');
   return
...
function oo = Import(o)                % Import Menu Items             
   ...
   ooo = mitem(oo,'Ultra1');
   oooo = mitem(ooo,'Ultra1 Data (.log)',{@ImportCb,'ReadUltra1Log','.log',@ultra1});
   oooo = mitem(ooo,'Ultra1 Data (.dat)',{@ImportCb,'ReadUltra1Dat','.dat',@ultra1});
   plugin(oo,'ultra1/shell/Import');
   return
...
function oo = Export(o)                % Export Menu Items             
   ...
   ooo = mitem(oo,'Ultra1');
   set(mitem(ooo,inf),'enable',onoff(o,{'log','pln','smp'},'ultra1'));
   oooo = mitem(ooo,'Ultra1 Log Data (.log)',{@ExportCb,'WriteUltra1Log','.log',@ultra1});
   set(mitem(oooo,inf),'enable',onoff(o,'log','ultra1'));
   oooo = mitem(ooo,'Plain Ultra1 (.dat)',{@ExportCb,'WriteUltra1Dat','.dat',@ultra1});
   set(mitem(oooo,inf),'enable',onoff(o,'pln','ultra1'));
   oooo = mitem(ooo,'Simple Ultra1 (.dat)',{@ExportCb,'WriteUltra1Dat','.dat',@ultra1});
   set(mitem(oooo,inf),'enable',onoff(o,'smp','ultra1'));
   
   plugin(oo,'ultra1/shell/Export');  % plug point
   return
...
function oo = Collect(o)               % Configure Collection          
   ...
   plugin(o,'ultra1/shell/Collect');   % plug point
   
   if isequal(class(o),'caramel')
      table = {{@read,'carabao','ReadStuffTxt','.txt'}};
      collect(o,{'weird','ball','cube'},table); 
   end
   if isequal(class(o),'ultra1')
      table = {{@read,'ultra1','ReadUltra1Dat','.dat'}};
      collect(o,{'pln','smp'},table); 
   end
   if isequal(class(o),'ultra1')
      table = {{@read,'ultra1','ReadUltra1Log','.log'}};
      collect(o,{'log'},table); 
   end
   if isequal(class(o),'ultra1')
      table = {{@read,'caramel','ReadPkgPkg','.pkg'},...
               {@read,'ultra1','ReadUltra1Dat','.dat'}};
      collect(o,{'pln','smp'},table); 
   end
   oo = o;
end
...
%
   ...
   plugin(oo,'ultra1/shell/Signal');   % plug point
   return
...
function oo = Analysis(o)              % Analysis Menu                 
   ...
   plugin(oo,'ultra1/shell/Analysis');
end
...
function oo = Simulation(o)            % Simulation Menu                 
   ...
   plugin(oo,'ultra1/shell/Simulation');
end
...
function oo = Info(o)                  % Info Menu                   
   ...
   oooo = mitem(ooo,['Ultra1 Class: Version ',version(ultra1)]);
   ooooo = mitem(oooo,'Edit Release Notes','edit ultra1/version');
   plugin(oo,'ultra1/shell/Info');
end
...
function oo = WriteUltra1Dat(o)         % Read Driver for ultra1 .dat  
   path = arg(o,1);
