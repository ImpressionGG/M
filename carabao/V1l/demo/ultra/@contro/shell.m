...
function o = Init(o)                   % Init Object                   
   ...
   o = set(o,{'title'},'Contro Shell');
   o = set(o,{'comment'},{'Playing around with CONTRO objects'});
...
function oo = New(o)                   % New Menu Items                
   ...
   ooo = mitem(oo,'Contro');
   oooo = mitem(ooo,'New Plain Contro',{@NewCb,'Plain','pln'});
   oooo = mitem(ooo,'New Simple Contro',{@NewCb,'Simple','smp'});
   plugin(oo,'contro/shell/New');
   return
...
function oo = Import(o)                % Import Menu Items             
   ...
   ooo = mitem(oo,'Contro');
   oooo = mitem(ooo,'Control Data (.log)',{@ImportCb,'ReadControlLog','.log',@contro});
   oooo = mitem(ooo,'Control Data (.dat)',{@ImportCb,'ReadControlDat','.dat',@contro});
   plugin(oo,'contro/shell/Import');
   return
...
function oo = Export(o)                % Export Menu Items             
   ...
   ooo = mitem(oo,'Contro');
   set(mitem(ooo,inf),'enable',onoff(o,{'log','pln','smp'},'contro'));
   oooo = mitem(ooo,'contro Log Data (.log)',{@ExportCb,'WriteControlLog','.log',@contro});
   set(mitem(oooo,inf),'enable',onoff(o,'log','contro'));
   oooo = mitem(ooo,'Plain contro (.dat)',{@ExportCb,'WritecontroDat','.dat',@contro});
   set(mitem(oooo,inf),'enable',onoff(o,'pln','contro'));
   oooo = mitem(ooo,'Simple contro (.dat)',{@ExportCb,'WritecontroDat','.dat',@contro});
   set(mitem(oooo,inf),'enable',onoff(o,'smp','contro'));
   
   plugin(oo,'contro/shell/Export');  % plug point
   return
...
function oo = Collect(o)               % Configure Collection          
   ...
   plugin(o,'contro/shell/Collect');   % plug point
   
   if isequal(class(o),'caramel')
      table = {{@read,'carabao','ReadStuffTxt','.txt'}};
      collect(o,{'weird','ball','cube'},table); 
   end
   if isequal(class(o),'contro')
      table = {{@read,'contro','ReadcontroDat','.dat'}};
      collect(o,{'pln','smp'},table); 
   end
   if isequal(class(o),'contro')
      table = {{@read,'contro','ReadcontroLog','.log'}};
      collect(o,{'log'},table); 
   end
   if isequal(class(o),'contro')
      table = {{@read,'caramel','ReadPkgPkg','.pkg'},...
               {@read,'contro','ReadcontroDat','.dat'}};
      collect(o,{'pln','smp'},table); 
   end
   oo = o;
end
...
%
   ...
   plugin(oo,'contro/shell/Signal');   % plug point
   return
...
function oo = Analysis(o)              % Analysis Menu                 
   ...
   plugin(oo,'control/shell/Analysis');
end
...
function oo = Simulation(o)            % Simulation Menu                 
   ...
   plugin(oo,'control/shell/Simulation');
end
...
function oo = Info(o)                  % Info Menu                   
   ...
   oooo = mitem(ooo,['Control Class: Version ',version(control)]);
   ooooo = mitem(oooo,'Edit Release Notes','edit control/version');
   plugin(oo,'control/shell/Info');
end
...
function oo = ReadControlDat(o)          % Read Driver for control .dat  
   path = arg(o,1);
   oo = read(o,'ReadGenDat',path);
   oo = Config(oo);                    % overwrite configuration
end
function oo = WriteControlDat(o)         % Read Driver for control .dat  
   path = arg(o,1);
