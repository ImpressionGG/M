...
function o = Init(o)                   % Init Object                   
   ...
   o = set(o,{'title'},'Ultra2 Shell');
   o = set(o,{'comment'},{'Playing around with ULTRA2 objects'});
...
function oo = New(o)                   % New Menu Items                
   ...
   ooo = mitem(oo,'Ultra2');
   oooo = mitem(ooo,'New Plain Ultra2',{@NewCb,'Plain','pln'});
   oooo = mitem(ooo,'New Simple Ultra2',{@NewCb,'Simple','smp'});
   plugin(oo,'ultra2/shell/New');
   return
...
function oo = Import(o)                % Import Menu Items             
   ...
   ooo = mitem(oo,'Ultra2');
   oooo = mitem(ooo,'Ultra2 Data (.log)',{@ImportCb,'ReadUltra2Log','.log',@ultra2});
   oooo = mitem(ooo,'Ultra2 Data (.dat)',{@ImportCb,'ReadUltra2Dat','.dat',@ultra2});
   plugin(oo,'ultra2/shell/Import');
   return
...
function oo = Export(o)                % Export Menu Items             
   ...
   ooo = mitem(oo,'Ultra2');
   set(mitem(ooo,inf),'enable',onoff(o,{'log','pln','smp'},'ultra2'));
   oooo = mitem(ooo,'Ultra2 Log Data (.log)',{@ExportCb,'WriteUltra2Log','.log',@ultra2});
   set(mitem(oooo,inf),'enable',onoff(o,'log','ultra2'));
   oooo = mitem(ooo,'Plain Ultra2 (.dat)',{@ExportCb,'WriteUltra2Dat','.dat',@ultra2});
   set(mitem(oooo,inf),'enable',onoff(o,'pln','ultra2'));
   oooo = mitem(ooo,'Simple Ultra2 (.dat)',{@ExportCb,'WriteUltra2Dat','.dat',@ultra2});
   set(mitem(oooo,inf),'enable',onoff(o,'smp','ultra2'));
   
   plugin(oo,'ultra2/shell/Export');  % plug point
   return
...
function oo = Collect(o)               % Configure Collection          
   ...
   plugin(o,'ultra2/shell/Collect');   % plug point
   
   if isequal(class(o),'caramel')
      table = {{@read,'carabao','ReadStuffTxt','.txt'}};
      collect(o,{'weird','ball','cube'},table); 
   end
   if isequal(class(o),'ultra2')
      table = {{@read,'ultra2','ReadUltra2Dat','.dat'}};
      collect(o,{'pln','smp'},table); 
   end
   if isequal(class(o),'ultra2')
      table = {{@read,'ultra2','ReadUltra2Log','.log'}};
      collect(o,{'log'},table); 
   end
   if isequal(class(o),'ultra2')
      table = {{@read,'caramel','ReadPkgPkg','.pkg'},...
               {@read,'ultra2','ReadUltra2Dat','.dat'}};
      collect(o,{'pln','smp'},table); 
   end
   oo = o;
end
...
%
   ...
   plugin(oo,'ultra2/shell/Signal');   % plug point
   return
...
function oo = Analysis(o)              % Analysis Menu                 
   ...
   plugin(oo,'ultra2/shell/Analysis');
end
...
function oo = Simulation(o)            % Simulation Menu                 
   ...
   plugin(oo,'ultra2/shell/Simulation');
end
...
function oo = Info(o)                  % Info Menu                   
   ...
   oooo = mitem(ooo,['Ultra2 Class: Version ',version(ultra2)]);
   ooooo = mitem(oooo,'Edit Release Notes','edit ultra2/version');
   plugin(oo,'ultra2/shell/Info');
end
...
function oo = WriteUltra2Dat(o)         % Read Driver for ultra2 .dat  
   path = arg(o,1);
