function [cpk,tout,rout,wout,yout,tfout,fout,kout] = simu(o,kind,type,Tf,Kp,Ns)
%
% SIMU      Simulation of several filters with specific type & parameter setups
%
%              simu(o,'twin',2,Tf,Kp,Ns)
%              simu(o,'kafi',{2 1},Tf,Kp)   % deterministic noise
%              simu(o,'twin')               % Tf = 3, det = 0, Kp = 0.5
%
%              cpk = simu(o,'twin',type,Tf,Kp)  % no plot
%              cpk = simu(o,'twin',{type,det,ref},Tf,Kp)  % no plot
%
...
% system parameters  & sampling time
   ...
   if (nargin < 2) error('filter kind (arg1) expected!'); end
   if (nargin < 3) type = 0;   end % filter type number
   if (nargin < 4) Tf = 3;     end % filter time constant
   if (nargin < 5) Kp = 0.5;   end % Kalman decay
   if (nargin < 6) Ns = 1;     end % number of simultaneous runs
   if (nargin < 7) Nx = 0; end % number of simultaneous runs
...
%     F2 = eval(cmd);               % setup enhanced twin filter
      F2 = gamma([type Tf Kp ns]);  % setup special filter
...
%        [yf2k,F2] = eval(cmd);      % Filter F2 operation
         [yf2k,F2] = gamma(F2,yk,tk);% Filter F2 operation
      ...
      % now log all recorded data into a log object

   put = opt(o,{'put',true});
   while (put)                            % if to put an object to shell
      oo = log(fito('fito'),['t,r[µ]:k#1,y[µ]:g#1,f0[µ]#1:b,f1[µ]#1:r,',...
               'f2[µ]:k#1,n[µ]:g(o)#2,q[µ]:m#2,e0[µ]:b#2,e1[µ]:r#2,e2[µ]:k#2']);
      t0 = 0;  n = y-r*ones(1,Ns);        % noise
      for (i=1:Ns)
         oo = log(oo);
         f0i = yf0(:,i);  f1i = yf1(:,i);  f2i = yf2(:,i); 
         ni = n(:,i)';  e0i = f0(:,i);   e1i = f1(:,i);   e2i = f2(:,i); 
         oo = log(oo,t0+t',r',y(:,i)',f0i',f1i',f2i',ni,q',e0i',e1i',e2i');
         t0 = t0 + max(t);
      end
      oo = set(oo,'std0',std0,'std1',std1,'std2',std2);
      oo = set(oo,'avg0',avg0,'avg1',avg1,'avg2',avg2);
      oo = set(oo,'cpk0',cpk0,'cpk1',cpk1,'cpk2',cpk2);

      c.Nx = Nx;  c.type = type;  c.Tf = Tf;  c.Kp = Kp;  c.stdw = stdw;
      c.F2 = F2;
      oo = set(oo,'context',c);

      plot(oo,'Put');
      break                            % this is an IF-WHILE ;-)
   end
