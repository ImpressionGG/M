...
%
   [gamma,oo] = manage(o,varargin,@Error,@Default,@Begin,@End,@File,...
                 ...
                 @Basket,@Kind,@Overlays,@Bias,@Toolbar,@Filter,@Scope,...
                 @Ignore,@Style,@Variation,@Scale,@Camera,@CamCb,@Grid,...
                 @Plot,@PlotCb,@Analysis,@Geometry,@GeoCb,@Surf,...
...
function oo = Begin(o)                 % Begin Menu Build (Overloaded) 
   ...
   co = cast(o,'carabao');
   oo = menu(co,'Begin');              % call carabao/menu/Begin
   busy(oo,'Building menu ...');
end
function oo = End(o)                   % End Menu Build (Overloaded)   
   co = cast(o,'carabao');
   oo = menu(co,'End');                % call carabao/menu/Begin
   ready(oo);
end
...
function oo = NewPlain(o)              % Create New plain trace        
   ...
   oo = caramel('plain');           % construct CARAMEL object
   ...
   oo = config(oo,'p',{2,'g',2});
      ...
      % paste object into shell and display caramel info on screen
...
function oo = NewSimple(o)             % Create New biased trace       
   ...
   oo = caramel('simple');             % construct CARAMEL object
...
function oo = PackageInfo(o)           % Provide Package Info File     
   ...
   oo.par.version = ['CARABAO ',version(o)];
   ...
   setting(o,'tools.sizes',get(oo,'sizes'));
   setting(o,'tools.method',get(oo,'method'));
   setting(o,'tools.pitch',get(oo,'pitch'));
   setting(o,'tools.system',get(oo,'system'));
   ...
   function list = Kinds(o)            % Get List of Kinds             
      ...
      if container(o)
         for (i=1:length(o.data))
            oo = o.data{i};
            kind = get(oo,'kind');
            if ~isempty(kind) && ~carabao.is(kind,list)
               list{end+1} = kind;
            end
         end
      end
...
%
   o = cast(o,'caramel');              % cast to caramel object
   oo = plot(o,'Setup');               % setup Plot menu
end
function o = PlotCb(o)                 % Plot Callback                 
   fprintf('*** warning: caramel/menu/PlotCb() will be obsoleted!\n');
   args = arg(o);                      % get args
   o = arg(o,[{'Plot'},args]);         % expand arg list with 'Plot' head
   o = cast(o,'caramel');              % cast to caramel object
   o = plot(o);                        % redirect to caramel/plot/Plot
   return
...
function oo = Analysis(o)              % Analysis Menu                 
   ...
   plugin(oo,'caramel/menu/Analysis'); % plug point
end
...
%
   ...
   method = either(get(o,'method'),'');
   pitch = either(get(o,'pitch'),'');
   if length(pitch) < 1
      pitch = '';
   elseif length(pitch) < 2
      pitch = sprintf('[%g %g]',pitch(1),pitch(1));
...
%
   prompts = {'Title','Comment','Date','Time','Project','Package','Kind',...
              'System','Sizes','Method','Pitch','Version','Creator'};
   ...
   sizes = eval(values{9},'NaN');
   if isa(sizes,'double') && length(sizes) == 2 || length(sizes) == 3
      if all(round(sizes(1:2)) == sizes(1:2) & sizes(1:2) > 0)  % positive integers!
   ...
   pitch = eval(values{11},'NaN');
   if isa(pitch,'double') && length(pitch) == 2
      oo = set(oo,'pitch',pitch);
   end
