...
%
% SETTING   Get or set figure/menu settings of current figure. 
%
%    These settings are stored as a structure in figure's user data. 
...
%       setting([])                  % clear all settings
%
%    Note: setting([]) clears all settings but maintains current 
%    figure object setting (=> gcfo)!
%
%    See also: CORE, GFO, DEFAULT, OPTION
...
% from here we only deal with new style calls
   ...
   elseif (nargin == 1)              % three possibilities a) b) and c)! 
       
      if (isstruct(name))            % a) refresh all settings!
         set(gcf,'userdata',name);
      elseif (isempty(name))         % b) clear all settings
         obj = gcfo;                 %    pull current figure object
         set(gcf,'userdata',[]);     %    clear 
         gcfo(obj);                  %    push back current figure object
         
      elseif (isstr(name))           % c) retrieve setting spec'd by name
         settings = get(gcf,'userdata');
