...
%    can be passed through a core object. These options are forwarded
%    to CORE/TEXT
%
%      smo = option(core,'position',[50,0]);
%      smo = option(core,'color','b');
%      smo = option(core,'valign','bottom');
%      smo = option(core,'halign','center');
%      smo = option(core,'figure',2);
%
...
%
   ...
   try
      ...
      % There are two cases:
      %
      % a) no core object provided. In this case we will use an existing 
      %    GAO handle, otherwise a new one will be setup with default 
      %    settings
      %
      % b) core object provided as first arg: the actual handle (if
      %    exists) will be deleted and a new busy text will be displayed
      %    according to the options of the core object
   ...
   if (busymessage)
      ...
      if isempty(smo)                % case a) no core object provided
         hdl = gao('busy');
         if isempty(hdl)             % ignore
            %smo = text(gao,msg,0.5,0.5,'size',16,'color','b');
            %hdl = arg(smo);
         else
            set(hdl,'string',underscore(core,msg));
         end
      else
         hdl = gao('busy');
         eval('delete(hdl);','');
