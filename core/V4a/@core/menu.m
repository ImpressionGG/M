...
%       menu(obj,'XInfo');      % add an extended INFO roll down menu
%
%
...
%       menu(obj,'Version')     % add a VERSION rolldown menu
...
%
   ...
   txt = either(get(obj,'title'),info(core(obj)));
...
function XInfo(obj)
%
% XINFO   Setup an extended Info roll down menu
%
   Info(obj,1);  % call info with extensions activated
   return
end

function Info(obj,extend)
%
% INFO   Setup INFO menu. There are two possibilities to call:
%
%           Info(obj,1)    % extended menue (expanded version menu)
%           Info(obj,0)    % basic menu (only invisible 'Version' header)
%
   default('profiler',0);
   default('shell.debug',0);
   
   if (nargin < 2)
      extend = either(arg(obj,1),0);
   end
   ...
   ob2 = mitem(ob1,'Version','','','visible','off');
   if (extend)
      Version(obj);    % expand Version menu
   end
      
   ob2 = mitem(ob1,'-');
   ob2 = mitem(ob1,'Tutorial',call('Tutorial'));
   ob2 = mitem(ob1,'How To',call('HowTo'));
   return
end
...
function HowTo(obj)
%
% HOW-TO   Print message how to get a figure's object into MATLAB work space
%
...
%
   ob1 = mitem(obj,{'Version'});
   ob2 = mitem(ob1,['Core Toolbox: Version ',version(core)]);
   ob3 = mitem(ob2,'Display Release Notes','help core/version');
   ob3 = mitem(ob2,'Edit Release Notes','edit core/version');
   return
end
...
function OldInfoMenu(obj,mountpt)
%
   LB = 'label';  CB = 'callback';  UD = 'userdata';  EN = 'enable'; 
   CHK = 'check(gcbo);';  CHKR = [CHK,call('refresh')];
   CHC = 'choice(gcbo);'; CHCR = [CHC,call('refresh')];
