function out = match(arg1,arg2,trunc)
%
...
%       idx = match('vA2',table)
%
%    The result is usually either empty ([]) if no match found or
%    a row vector of indices if a match occured. Applying a third 
%    argument (truncate) will truncate the row vector of indices
%    to the first element of indices, which is according to the
%    first match found in the list.
%
%       i = match('vA1',list,1)   % only 1 index (or [])
%       i = match('vA1',table,1)  % only 1 index (or [])
%
%    See also: CORE
%
   truncate = (nargin >= 3);
   ...
   if (nargin == 1) % then calculate match table
      ...
      if length(str) > n
         ...
         % truncate index if necessary

      if (truncate & ~isempty(idx))
         idx = idx(1);                  % take first index
      end
