...
%       axes(hax(idx));                 % select axes(hax(idx)) as current
      ...
      hdl = plot(hax(idx),xx,yy,'color',color(clist{i}));
      hold(hax(idx),'on'); % hold on;
      if (option(obj,'bullets'))      % plot also black bullets?
         plot(hax(idx),xx,yy,'k.');
      end
      
      if (~isempty(xlim)) 
          set(hax(idx),'xlim',xlim{i});
      else
          if (max(x) > min(x))       % avoid crash
             set(hax(idx),'xlim',[min(x),max(x)]);
          end
      end
      if (~isempty(ylim)), set(hax(idx),'ylim',ylim{i});  end
      if (~isempty(linw)), set(hdl,'linewidth',linw{i});  end
...
function handlegrid(obj,hax)  % set grid on/off for all specified axes handles
   ...
   for i=1:length(hax),
      ...
      if (spec)
         ...
         ylim = get(hax(i),'ylim');
%          axes(hax(i));
         hdl = plot(hax(i),xlim,spec(1)*[1 1],'r-.', xlim,spec(2)*[1 1],'r-.');
         set(hdl,'linewidth',2);
