...
%    VERBOSE_INFO = 1;       % print initial info for parsing
   ...
   if (nargin < 4), error('compose(): at least 3 args expected!'); end
...
%    t0 = [];          % init
   ...
   gantry = [];                         % empty table for gantry numbers
   ...
   for listidx = 1:length(list),   % repeat for all records
      ...
      entry = list{listidx};
      sym = entry{1};  val = entry{2};  tnew = entry{3};  gnmb =entry{4};
      if (verbose >= 2) 
          token = entry{5};
          fprintf([sym,' %14.8f %14.8f   @%14.8f  %g  %s\n'],...
             val(1),val(2),tnew,gnmb,token);
      end
      ...
      if isempty(idx)
         ...
         gantry(idx) = gnmb;                 % mind gantry number in table
      end
      ...
      clash = ~isempty(find(collect==idx, 1));
   ...
   data = addrecord(data,table,gantry);    % trim data
...
%               data = addrecord(data,table,gantry);       % complete data
...
%
   ...
   elseif (nargin == 3)                   % trim & complete data
      ...
      gantry = record;                    % here arg3 is gantry
      ...
      data.gantry = gantry;               % add gantry number table
      return
   end
...
%
   ...
   if (nargin == 1) % then calculate match table
      ...
      if length(str) > n
         ...
         strtab = char(ones(m,1)*search);
         idx = find(~sum(abs(table-strtab),2))';
      end
      out = idx;
   end
