...
if ~(strcmpi('off',varargin{1}) && (narg==1)),
...
% Parse other arguments to TEXTBOX
     ...
     if ~(ischar(S))         %   Matrix of handles
    
         Kids = reshape(S,numel(S),1); 
              ...
              for i = (shift+3):(shift+2+size(Kids,1)-size(lstrings,1)),
                    if (i <= narg),
                          if ischar(varargin{i}),
                               lstrings = char(lstrings,varargin{i});
                          else
                               lstrings = char(lstrings,'');
                          end
                    else
                          lstrings = char(lstrings,'');
                    end
         ...
         for i=1:nstack,
              linetype = char(linetype,get(Kids(i),'LineStyle'));
              ...
              marker = char(marker,get(Kids(i),'Marker'));
     ...
     elseif mode == 2,   % Every other argument is a linespec
          ...
          warning(['The syntax TEXTBOX(linetype1,string1,linetype2,',...
          ...
          for i=(shift+1):2:(narg+tolflag)        
                ...
                if (isempty(msg) && ~isempty(lnstr))
                     ...
                     if (isempty(lnt))
                          linetype=char(linetype,defaultlinestyle);
                     else
                          linetype=char(linetype,lnt);
                     end
                     if (isempty(lnc))
                     ...
                     if (isempty(lnm)),
                          marker=char(marker,defaultlinemarker);
                     else
                          marker=char(marker,lnm);
                     end
...
hl=axes('units','normalized','position',lpos,'box','on', ...
      'nextplot','add','xtick',-1,'ytick',-1, ...
      'xticklabel','','yticklabel','','xlim',[0 1],'ylim',[0 1], ...
      'clipping','on','color',acolor,'tag','TEXTBOX',...
      'climmode',get(ha,'climmode'),'clim',get(ha,'clim'));
if isprop(hl,'SortMethod'),
  set(hl,'SortMethod','childorder');
end
...
if ~isempty(oldposdata)
    ...
    ud.posdata = [1.1, double(ha), cap, sticky];    % Position data
end
...
axpos = find(ord==ha);
legpos = find(ord==hl);
legposHelp = ord(legpos);
if legpos > axpos
    ord(legpos) = [];
    ord = [ord(1:axpos-1);legposHelp;ord(axpos:end)];
end
set(hf,'children',ord);
