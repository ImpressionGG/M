...
%
   ...
   if (isempty(mode)),
      mode = 6;
   end    % default filter mode
   
   [m,n] = size(M);
   
   if ( mode < 6 )
      if (q<0 || q>1),
         error('arg3 must be in the range 0..1!');
      end
   end
...
% mode 4: horizontal 1x3 filter operation
   ...
   if (mode == 4)
      if (n < 3),
         error('arg0 must have at least 3 columns');
      end
...
% mode 5: vertical 3x1 filter operation
   ...
   if (mode == 5)  % vertical 1x3 filtering
      if (m < 3),
         error('arg0 must have at least 3 rows');
      end
...
% mode 6: second order surface fitting
   ...
   if (mode == 6)
      if (m < w || n < w),
         error('arg0 must be at least %g x %g!',w,w);
      end
