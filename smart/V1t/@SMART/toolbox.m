...
function vers = getvers
%
% GETVERS  get version
%
   path = upper(which('smart/toolbox'));
   idx = findstr(path,'\@SMART\TOOLBOX.M');
   path(idx:end) = '';
   idx = findstr(path,'\SMART\V');
