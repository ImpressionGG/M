...
%           See also: SMART DEFAULT ADDON SETTING
   ...
   elseif (nargin == 2)             % two possibilities a) and b)! 
      ...
      if (isstruct(options))        % a) refresh all options!
          
         obj = set(obj,'options',options);
         value = obj;
         
      elseif ischar(name),          % b) retrieve specified option by name
...
%          end
      ...
      elseif (iscell(name))  % list of option settings
...
%          eval('arg2 = name');
      ...
      par.options.(name)=value; % eval(['par.options.',name,'=value;']);
      value = set(obj,par);
