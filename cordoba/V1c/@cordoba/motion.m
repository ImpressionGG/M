...
%
%             motion(cordoba)          % register plugin
%
...
%
%          See also: CORDOBA, PLUGIN, PLUG, BASIS, KEFALON, MOTION
%
   if (nargin == 0)
...
function o = Register(o)               % Plugin Registration           
   ...
   plugin(o,['cordoba/shell/Analysis'],{mfilename,'Analysis'});
end
...
function oo = Import(o)                % Import Menu Items             
   oo = mitem(o,'Motion');
   ooo = mitem(oo,'Motion Trace (.dat)',{@ImportCb,'ReadMotionDat','.dat',@cordoba});
   return
...
function oo = Export(o)                % Export Menu Items             
   ...
   ooo = mitem(oo,'Motion Trace (.dat)',{@ExportCb,'WriteMotionDat','.dat',@cordoba});
   return
...
function o = Analysis(o)               % Analysis Menu                 
   ...
   if ~isempty(oo)                     % if 'motion' typed CORDOBA object
      oo = mhead(o,'Motion');
   ...
   plugin(o,'cordoba/motion/Analysis');% plug point
end
...
function oo = MotionObject(o)          % Get Motion Object             
   oo = current(o);
   ctype = type(oo);
   if ~(isequal(ctype,'motion') && isa(oo,'cordoba'))
      oo = [];                         % no 'motion' typed CORDOBA object
   end
end
...
%
   ...
   comment{2} = sprintf('tmax = %g ms, Ts = %g ms',tmax,Ts);   
      ...
      % create motion object and convert to 'motion' typed CORDOBA
      
   t = [0:Ts:tmax+eps] / 1000;         % convert from ms to sec
   o = motion(corasim,{smax,t},vmax,amax,[tj/1000 algo],unit);       
   
   oo = cordoba('motion');
   oo = set(oo,'kind',kind);           % need to check for virgin
   
   oo = log(oo,'t','s','v','a','j');   % create CORDOBA based log object
...
%              oo = motion(cordoba,{smax,0:Ts:tmax},vmax,amax,tj,unit,algo);    
