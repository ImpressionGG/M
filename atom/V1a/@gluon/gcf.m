...
%
%       gcf(gluon,hGui)    % register GUI handle
%
...
%          gcf(gluon,handles.figure1);     % register GUI figure handle
%            :
%       end
%
...
%          try          
%             gcf(gluon,handles.figure1);    % register GUI figure handle
%          catch             % avoid a crash if quark is not available
%          end               % try/catch allows to run GUI without GLUON
%           :
%       end
%
%    Code lines: 40
%
%    See also: GLUON, FIGURE, PUSH, PULL
%
   if (nargin == 1)
