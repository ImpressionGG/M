...
%
%       util(gluon);         % setup: either, iif, is, cuo, sho, ticn, tocn
%    
%    Assign utility functions
%
%       [iif,either..] = gluon.util('iif','either',..)  % return function hdls
%
%    Utility functions:
%       color                % color setting
...
function Shorthands(o)                 % Setup some short hands
   evalin('base','iif=@gluon.iif;');
   evalin('base','is=@gluon.is;');
   evalin('base','either=@gluon.either;');
%  evalin('base','cuo=@gluon.cuo;');
%  evalin('base','sho=@gluon.sho;');
   evalin('base','ticn=@gluon.ticn;');
   evalin('base','tocn=@gluon.tocn;');
end
...
%
   ...
   ok = gluon.either(any(IsGraphics(hdl)),0) || isa(hdl,'double');
   if ~ok
       cla = class(hdl);
...
%    USCORE function.
%            
%       txt = uscore(carabao,'sample_text');
%
%    Try this:
%       title('sample_text');
%       title(uscore(carabao,'sample_text'));
%
%    The USCORE functionality can be deactivated by clearing
%    option 'underscore', e.g.
%
%       o = opt(carabao,'underscore',0);
