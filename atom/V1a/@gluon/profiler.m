...
%
%    First arg is a gluon object which has so far no meaning except enable
...
%    See also: GLUON
%
   global GlobGluonProfileBypass
   
   if (GlobGluonProfileBypass)
      if (nargin >= 2)
         if nargout > 0
            out = [];
         end
...
%     if mode == 0 | isempty(p) | eval('p.level','0') < 1
      ...
      p.trace = sprintf([symbol,':%10.6f s'],dt);
         ...
         % final refresh of accumulated overhead & GlobGluonProfile
         % note: 0.29 ms extra time for creating GLUON object
...
%
   ...
   p = profile(gluon,'main',0);     % reset & begin profiling main
   p = profile(gluon,'sub1',1);     % begin profiling sub1
   p = profile(gluon,'sub1',-1);    % end profiling sub1
   p = profile(gluon,'sub2',1);     % begin profiling sub1
   p = profile(gluon,'sub2',-1);    % end profiling sub1
   p = profile(gluzon,'main',-1);    % reset & begin profiling main
...
%
   ...
   p = profile(gluon,'main',-1)    % reset & begin profiling main
   time = p.time
