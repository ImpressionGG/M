...
%
   ...
   switch nargin
      case 1
         settings = shelf(gob,fig,'settings');
         if (nargout == 0)
            display(o,settings);
         else
            value = settings;
      ...
      case 2
         if ischar(tag)
            value = shelf(gob,fig,['settings.',tag]); % recall from shelf @ 'settings'
         elseif iscell(tag)
         ...
         elseif isempty(tag)
            shelf(gob,fig,'settings',[]);     % clear all settings
         elseif isstruct(tag)
            shelf(gob,fig,'settings',tag);    % init settings with struct
         end
      case 3
         if iscell(tag)
            tag = tag{1};
            if isempty(setting(o,tag))
               shelf(gob,fig,['settings.',tag],value); % change setting @ tag
            end
         else
            shelf(gob,fig,['settings.',tag],value); % change setting @ tag
         end
