...
%    See also: QUARK
%
   gluon.profiler('manage',1);
   [gamma,oo,name] = Lambda(o,list,varargin); % get proper gamma function
   if ~isempty(gamma)
      oo = balance(oo);
      gluon.profiler('manage',0);
      return
   end
%
...
%  super = superclasses(class(o));
   super = [];  % a dirty hack as long there is no superclass implementation
...
function [gamma,oo,name] = Lambda(o,list,flist)
%
% LAMBDA   Get proper function handle
%
