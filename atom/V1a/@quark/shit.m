...
% SHIT   Shit Animation   
%
%    Shows animated carabao shit which can be parametrized by roughness
%    and color
%
...
%
   ...
   if isempty(X)
      ...
      Y = Y + 0.1*U;  Z = Z + 0.1*V;
      o = work(o,'X',X);  o = work(o,'Y',Y);  o = work(o,'Z',Z);
   end
end
...
%
   X = work(o,'X');  X = X + rough*randn(size(X))/250;
   Y = work(o,'Y');  Y = Y + rough*randn(size(Y))/250;
   Z = work(o,'Z');  Z = Z + rough*randn(size(Z))/250;  
