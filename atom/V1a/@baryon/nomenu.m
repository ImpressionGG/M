function oo = menu(o,varargin)         % Carabao Menu Building Blocks  
% 
...
%
%        See also: CARABAO, SHELL, MITEM, CHECK, CHOICE, CHARM
%
   [gamma,o] = manage(o,varargin,@Begin,@End,@Dynamic,@Bar,@File,@Open,...
                     ...
                     @Title,@Home,@About,@What,@Page,@Gallery,...
...
function oo = Begin(o)                 % Begin Menu Build (Overloaded) 
   ...
   oo = cast(o,'caracow');             % cast to Caracow object
   oo = menu(oo,'Begin');              % call menu@caracow>Begin
...
function oo = End(o)                   % End Menu Build (Overloaded)
   oo = caracow(o);                    % cast to Caracow object
   oo = menu(oo,'End');                % call menu@caracow>End
end
...
function oo = Open(o)                  % Add Open Menu Item            
   ...
   function o = OpenCallback(o)        % Open Callback                 
      try
         bag = load(caracow);          % load an object, using CARACOW load
...
function oo = Save(o)                  % Add Save Menu Item            
   ...
   function o = SaveCallback(o)        % Save Callback                 
      ...
      try
         save(caracow,pack(o));        % save object, using CARACOW save
      catch err
...
function o = EditCb(o)                % Edit Property                 
...
function oo = CopyCb(o)                % Copy Object                   
   ...
   if isempty(list)
      ...
      clip(o,data(carabao,list));      % put basket list into clip board
   end
...
%       else
%          clip(o,data(carabao,{oo})); % put object into clip board
%       end
end
...
%       if isequal(mode,'Cut')
      clip(o,data(carabao,list));      % put object into clip board
%       end
...
%
   iif = @carabull.iif;                % need some utility
   ...
   function oo = Edit(o)               % Edit Info                     
      ...
      switch mode
         ...
         case 'About'
            ...
            oo = Property(oo);         % edit object properties
            if ~isempty(oo)
               current(carabao,oo);    % update current object
...
%
   oo = mhead(o,'Version');
   ooo = mitem(oo,['Carabao Toolbox: Version ',version(carabao)]);
   oooo = mitem(ooo,'Edit Release Notes','edit carabao/version');
end
...
function oo = Home(o)                  % Display Shell Home Info       
   ...
   color = carabull.either(control(o,'color'),0.9*[1 1 1]);
