...
%
%             sample(caramel)          % register plugin
%
...
%
%          See also: CARAMEL, PLUGIN, PLUG, BASIS, KEFALON
%
   if (nargin == 0)
...
function o = New(o)                    % New Sample Trace              
   ...
   function oo = NewPlain(o)           % Create New Plain Object       
      co = caramel(o);                 % convert to caramel object
   ...
   function o = ConfigPlain(o)         % Configure Plain Object        
      o = category(o,1,[-5 5],[0 0],'µ');      % category 1 for x,y
   ...
   function o = ConfigSimple(o)        % Configure Simple Object       
      o = category(o,1,[-5 5],[0 0],'µ');      % category 1 for x,y
      o = category(o,2,[-50 50],[0 0],'m°');   % category 2 for p
      o = category(o,3,[-0.5 0.5],[0 0],'µ');  % category 3 for ux,uy
...
function oo = Import(o)                % Import Menu Items             
   oo = mitem(o,'Sample');
   ooo = mitem(oo,'Plain Sample (.dat)',{@ImportCb,'ReadPlnDat','.dat',@caramel});
   ooo = mitem(oo,'Simple Sample (.dat)',{@ImportCb,'ReadSmpDat','.dat',@caramel});
   return
...
function oo = Export(o)                % Export Menu Items             
   ...
   ooo = mitem(oo,'Plain Sample (.dat)',{@ExportCb,'WritePlnDat','.dat',@caramel});
   set(mitem(ooo,inf),'enable',onoff(o,'pln'));
   ooo = mitem(oo,'Simple Sample (.dat)',{@ExportCb,'WriteSmpDat','.dat',@caramel});
